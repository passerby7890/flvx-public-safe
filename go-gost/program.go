...
func (p *program) Start() error {
	cfg, recovered, err := parseRuntimeConfigWithRecovery()
	if err != nil {
		return err
	}
	if recovered {
		logger.Default().Warn("runtime gost.json was invalid, restored automatically before startup")
	}

	cfg, repaired := sanitizeRuntimeConfig(cfg)
	if repaired {
		logger.Default().Warn("detected duplicated/stale service entries in gost.json, auto-repaired before startup")
		if err := socket.PersistSanitizedConfig(runtimeConfigPath(), cfg); err != nil {
			logger.Default().Warnf("persist repaired gost config failed: %v", err)
		}
	}
...
func (p *program) reloadConfig() error {
	...
	if recovered {
		logger.Default().Warn("runtime gost.json was invalid, restored automatically before reload")
	}
	cfg, repaired := sanitizeRuntimeConfig(cfg)
	if repaired {
		logger.Default().Warn("detected duplicated/stale service entries in gost.json during reload, auto-repaired")
		if err := socket.PersistSanitizedConfig(runtimeConfigPath(), cfg); err != nil {
			logger.Default().Warnf("persist repaired gost config failed: %v", err)
		}
	}
...
func parseRuntimeConfigWithRecovery() (*config.Config, bool, error) {
	cfg, err := parser.Parse()
	if err == nil {
		return cfg, false, nil
	}

	recovered, recoverErr := socket.RecoverRuntimeConfig(runtimeConfigPath())
	if recoverErr != nil {
...
func sanitizeRuntimeConfig(cfg *config.Config) (*config.Config, bool) {
	return socket.SanitizeConfigForRuntime(cfg)
}
