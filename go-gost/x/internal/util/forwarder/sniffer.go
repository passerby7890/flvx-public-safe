...
const (
	tlsALPNSelectHeader    = "X-Gost-Tls-Alpn"
	tlsBrowserSelectHeader = "X-Gost-Tls-Browser"
)
...
func (h *Sniffer) serveH2(ctx context.Context, conn net.Conn, ho *HandleOptions) error {
	...
	tr := &http2.Transport{
		DialTLSContext: func(ctx context.Context, network, addr string, cfg *tls.Config) (net.Conn, error) {
			node, cc, err := h.dialTLS(ctx, addr, nil, ho)
			if err != nil {
				return nil, err
			}
...
func (h *Sniffer) HandleTLS(ctx context.Context, conn net.Conn, opts ...HandleOption) error {
	...
	node, cc, err := h.dialTLS(ctx, host, clientHello.SupportedProtos, &ho)
	if err != nil {
		return err
	}
	defer cc.Close()
...
func (h *Sniffer) dialTLS(ctx context.Context, host string, alpns []string, ho *HandleOptions) (node *chain.Node, cc net.Conn, err error) {
	dial := ho.Dial
	if dial == nil {
		dial = (&net.Dialer{}).DialContext
	}
	...
	var triedNodes []string
	var lastErr error
	selectHeader := tlsSelectHeader(alpns)
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if ho.Hop != nil {
			node = ho.Hop.Select(selectCtx,
				...
				hop.HeaderSelectOption(selectHeader),
			)
		}
		if node == nil {
			if lastErr != nil {
...
func tlsSelectHeader(alpns []string) http.Header {
	header := http.Header{}
	for _, alpn := range alpns {
		...
		header.Add(tlsALPNSelectHeader, alpn)
	}
	if isBrowserALPN(alpns) {
		header.Set(tlsBrowserSelectHeader, "1")
	}
	return header
}

func isBrowserALPN(alpns []string) bool {
	if len(alpns) == 0 {
		return false
	}
	hasHTTP := false
