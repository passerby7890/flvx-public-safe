...
func TestNormalizeTLSProbeServerName(t *testing.T) {
	if got := normalizeTLSProbeServerName("example.com", ""); got != "example.com" {
		t.Fatalf("expected host fallback, got %q", got)
	}
	if got := normalizeTLSProbeServerName("1.2.3.4", ""); got != "" {
		t.Fatalf("expected empty server name for literal ip, got %q", got)
	}
	if got := normalizeTLSProbeServerName("example.com", "edge.example.com"); got != "edge.example.com" {
		t.Fatalf("expected explicit server name, got %q", got)
	}
}
