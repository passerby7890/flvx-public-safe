...
func updateServices(req updateServicesRequest) error {
	...
	for i := range req.Data {
		...
		if old != nil {
			...
			if oldConfig != nil && oldConfig.Addr != "" {
				_ = kill.ForceClosePortConnections(oldConfig.Addr)
...
func deleteServices(req deleteServicesRequest) error {
	...
	for _, std := range servicesToDelete {
		...
		std.service.Close()
		if serviceConfig := serviceConfigs[std.name]; serviceConfig != nil && serviceConfig.Addr != "" {
