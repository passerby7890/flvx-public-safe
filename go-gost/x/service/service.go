...
func (s *defaultService) Serve() error {
	...
	for {
		...
		go func() {
			...
			if shouldWrapConnDetection(s.name) {
...
func (s *defaultService) Close() error {
	...
	// Close the listener first so the bound port is released immediately.
	// Some handlers may spend time draining goroutines/connections on Close(),
	// and keeping the listener open until then causes follow-up Add/Update
	// operations to race with the old bind and hit "address already in use".
	listenerErr := s.listener.Close()

	if closer, ok := s.handler.(io.Closer); ok {
		if err := closer.Close(); err != nil && listenerErr == nil {
			listenerErr = err
		}
	}
	return listenerErr
...
func shouldWrapConnDetection(serviceName string) bool {
	if !needWrap {
		return false
	}
	return !isPanelManagedForwardService(serviceName)
}

func isPanelManagedForwardService(serviceName string) bool {
	name := strings.TrimSpace(serviceName)
	if name == "" {
		return false
	}
	if strings.HasPrefix(name, "shared_sni_") {
		return true
	}
