import type {
  ...
  ForwardApiItem,
  ForwardSLAStatusSummaryApiItem,
  SpeedLimitApiItem,
} from "@/api/types";
...
import {
  createForward,
  getForwardEntryStatusBatch,
  getForwardTargetStatusBatch,
  getForwardList,
  getSpeedLimitList,
  getPeerShareList,
  ...
  getTunnelList,
  getNodeList,
  getForwardEntryStatus,
  getForwardSLAStatusBatch,
  pauseForwardService,
...
interface ForwardEntryHealthSummary {
  total: number;
  healthy: number;
  overallStatus: "healthy" | "partial" | "failed" | "unknown";
  items?: Array<{
    nodeId: number;
    nodeName: string;
    address: string;
    ...
    reachable: boolean;
    healthy: boolean;
    occupiedByExternal: boolean;
    reason?: string;
  }>;
}

interface ForwardTargetHealthSummary {
  total: number;
  healthy: number;
  overallStatus: "healthy" | "partial" | "failed" | "unknown";
  items?: Array<{
...
type ForwardEntryHealthFetchState = "idle" | "loading" | "ready" | "failed";
...
type ForwardMode = Forward["mode"];

interface ForwardModeGroup {
  mode: ForwardMode;
  items: Forward[];
}
...
const ENTRY_HEALTH_REFRESH_INTERVAL_MS = 60000;
const FORWARD_MODE_ORDER: ForwardMode[] = ["sni", "direct"];
...
const buildForwardModeGroups = (items: Forward[]): ForwardModeGroup[] => {
  const grouped = new Map<ForwardMode, Forward[]>();

  items.forEach((item) => {
    const mode: ForwardMode = item.mode === "sni" ? "sni" : "direct";
    const currentItems = grouped.get(mode);
    ...
    grouped.set(mode, [item]);
  });

  return FORWARD_MODE_ORDER.map((mode) => {
    const modeItems = grouped.get(mode) || [];
  ...
  }).filter((group): group is ForwardModeGroup => Boolean(group));
};

const getForwardModeGroupLabel = (mode: ForwardMode): string => {
  return mode === "sni" ? "SNI 共享入口" : "直连转发";
};

const getForwardModeGroupDescription = (mode: ForwardMode): string => {
  return mode === "sni"
    ? "共享端口按 SNI 分流，可单独全选这一组"
    : "普通端口转发，不会连带选择 SNI 规则";
};

const getForwardModeGroupChipColor = (mode: ForwardMode) => {
  return mode === "sni" ? "primary" : "default";
};
...
const mapForwardApiItems = (items: ForwardApiItem[]): Forward[] => {
  return (items || []).map((forward) => ({
    ...
    mode: forward.mode === "sni" ? "sni" : "direct",
...
const sniHostPattern =
  /^(\*\.)?([a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)(\.[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;

const parseSniRulesInput = (value: string): ParsedSniRule[] => {
  return value
    .split(/\r?\n/)
    .map((line) => {
    ...
    .filter((rule): rule is ParsedSniRule => Boolean(rule));
};

const isValidSniRuleHost = (host: string): boolean => {
  const normalized = host.trim();
  return normalized.length > 0 && !normalized.endsWith(".") && sniHostPattern.test(normalized);
};

const getSniRuleCount = (value: string): number => parseSniRulesInput(value).length;

const getForwardRuleCount = (form: ForwardForm): number => {
  if (form.mode === "sni") {
    return getSniRuleCount(form.sniRules);
  }

  return form.remoteAddr
    .split("\n")
    .map((addr) => addr.trim())
    .filter((addr) => addr).length;
};

const getForwardTargetSummary = (forward: Pick<Forward, "mode" | "remoteAddr" | "sniRules">): string => {
  const remote = formatRemoteAddress(forward.remoteAddr);
  if (forward.mode !== "sni") {
    return remote;
  }

  const rules = parseSniRulesInput(forward.sniRules);

  if (rules.length === 0) {
    return "SNI dispatch";
  }

  const first = `${rules[0].host} -> ${remote}`;
...
const getForwardTargetSearchText = (
  forward: Pick<Forward, "mode" | "remoteAddr" | "sniRules">,
): string => {
  if (forward.mode === "sni") {
    return `${forward.sniRules}\n${forward.remoteAddr}`.trim();
  }

  return forward.remoteAddr;
};

const getSniDeploymentDisplay = (
  forward: Pick<Forward, "mode" | "status" | "sniRules" | "inPort">,
) => {
  if (forward.mode !== "sni") {
    return null;
  }

  const ruleCount = getSniRuleCount(forward.sniRules);
  const portLabel = forward.inPort ? `:${forward.inPort}` : "";

  switch (forward.status) {
    case 1:
      return {
        color: "success" as const,
        text: `SNI${portLabel} 已部署 ${ruleCount} 条`,
      };
    case 0:
      return {
        color: "warning" as const,
        text: `SNI${portLabel} 已暂停`,
      };
    case -1:
      return {
        color: "danger" as const,
        text: `SNI${portLabel} 部署异常`,
      };
    default:
      return {
        color: "default" as const,
        text: `SNI${portLabel} 等待确认`,
      };
  }
};

const formatRelativeTimeLabel = (timestamp: number | null): string => {
  if (!timestamp) {
    return "尚未完成首轮检测";
  }
...
const ForwardEntryStatusChip = ({
  forwardId,
  getEntryHealthDisplay,
  className,
}: any) => {
  const display = getEntryHealthDisplay(forwardId);
    ...
    return (
      <div className={`space-y-1 ${className || ""}`.trim()}>
        <div className="flex flex-wrap items-center gap-1">
          ...
          <Chip
            className="h-5 px-1 text-[11px] font-medium"
            color={display.unhealthy > 0 ? "danger" : "success"}
            size="sm"
            variant="flat"
          >
            异常 {display.unhealthy}
          </Chip>
        </div>
        {display.detail ? (
          <div className={`truncate text-[11px] leading-4 ${detailClassName}`}>
            {display.detail}
          </div>
        ) : null}
      </div>
...
const ForwardTargetStatusChip = ({
  forwardId,
  getTargetHealthDisplay,
  className,
}: any) => {
  const display = getTargetHealthDisplay(forwardId);
  const detailClassName =
    display.color === "danger"
      ? "text-danger-600"
      : display.color === "warning"
...
const ForwardSniDeploymentChip = ({
  forward,
  className,
}: {
  forward: Pick<Forward, "mode" | "status" | "sniRules" | "inPort">;
  ...
  const display = getSniDeploymentDisplay(forward);
  ...
  return (
    <Chip
      className={`h-5 px-1 text-[11px] font-medium ${className || ""}`.trim()}
      color={display.color as any}
      size="sm"
      variant="flat"
    >
...
const ForwardSLAStatusChip = ({
  forwardId,
  getSLADisplay,
  className,
}: {
  forwardId: number;
  getSLADisplay: (forwardId: number) => {
  ...
  const display = getSLADisplay(forwardId);
  const detailClassName =
    display.color === "danger"
      ? "text-danger-600"
      : display.color === "warning"
...
const getEntryHealthPanelClassName = (display: {
  color: "success" | "warning" | "danger" | "default";
}) => {
  switch (display.color) {
    case "success":
      return "border-success-200 bg-success-50/80 text-success-700 hover:bg-success-100/80 dark:border-success-500/30 dark:bg-success-950/20";
    case "warning":
      return "border-warning-200 bg-warning-50/80 text-warning-700 hover:bg-warning-100/80 dark:border-warning-500/30 dark:bg-warning-950/20";
    case "danger":
      return "border-danger-200 bg-danger-50/80 text-danger-700 hover:bg-danger-100/80 dark:border-danger-500/30 dark:bg-danger-950/20";
    default:
      return "border-default-200 bg-default-100/50 text-default-700 hover:bg-default-200 dark:border-default-300 dark:bg-default-100/50";
  }
};
...
const SortableTableRow = ({
  ...
  getSLADisplay,
  formatInAddress,
  formatRemoteAddress: _formatRemoteAddress,
  handleServiceToggle,
  handleEdit,
  handleDelete,
  handleDiagnose,
  ...
  const entryHealthDisplay = getEntryHealthDisplay(forward.id);
  const entryPanelClassName = getEntryHealthPanelClassName(entryHealthDisplay);
    ...
    <TableRow key={forward.id} ref={setNodeRef} style={style}>
      ...
      >
        <div className="space-y-1">
          <button
            className={`w-full truncate rounded-md border px-2.5 py-1.5 text-left font-mono text-xs font-medium transition-all hover:shadow-sm cursor-pointer ${entryPanelClassName}`}
          ...
          <ForwardEntryStatusChip
            className="max-w-full"
            forwardId={forward.id}
            getEntryHealthDisplay={getEntryHealthDisplay}
          />
        </div>
      </TableCell>
      <TableCell
      ...
      >
        <div className="space-y-1">
          <button
            ...
            title={getForwardTargetSummary(forward)}
            type="button"
            onClick={() => showForwardTargetModal(forward)}
          >
            {getForwardTargetSummary(forward)}
          </button>
          <ForwardTargetStatusChip
            className="max-w-full"
            forwardId={forward.id}
            getTargetHealthDisplay={getTargetHealthDisplay}
          />
          <ForwardSniDeploymentChip forward={forward} className="max-w-full" />
        </div>
      </TableCell>
    ...
    <TableRow key={forward.id} ref={setNodeRef} style={style}>
      ...
      >
        <div className="space-y-1">
          <button
            className={`w-full truncate rounded-md bg-default-100/50 px-2.5 py-1.5 text-left font-mono text-xs font-medium text-default-700 transition-all ${
              forward.mode !== "sni" && hasMultipleAddresses(forward.remoteAddr)
                ? "hover:bg-default-200 hover:shadow-sm cursor-pointer"
                : "cursor-default"
            }`}
            title={getForwardTargetSummary(forward)}
...
export default function ForwardPage() {
  ...
  const effectiveViewMode = isMobile ? "direct" : viewMode;
  const useCompactTableLayout = compactMode && !isMobile;
  ...
  const [forwardEntryHealthMap, setForwardEntryHealthMap] = useState<
    Record<number, ForwardEntryHealthSummary>
  >({});
  const [forwardEntryHealthStateMap, setForwardEntryHealthStateMap] = useState<
    Record<number, ForwardEntryHealthFetchState>
  >({});
  const [forwardTargetHealthMap, setForwardTargetHealthMap] = useState<
    Record<number, ForwardTargetHealthSummary>
  >({});
  const [forwardTargetHealthStateMap, setForwardTargetHealthStateMap] =
    useState<Record<number, ForwardEntryHealthFetchState>>({});
  const [forwardSLAStatusMap, setForwardSLAStatusMap] = useState<
    Record<number, ForwardSLAStatusSummaryApiItem>
  >({});
  const [entryHealthRequestInFlight, setEntryHealthRequestInFlight] =
    useState(false);
  const [lastEntryHealthCheckedAt, setLastEntryHealthCheckedAt] = useState<
    number | null
  >(null);
  const [entryHealthCountdownSec, setEntryHealthCountdownSec] = useState(
    Math.ceil(ENTRY_HEALTH_REFRESH_INTERVAL_MS / 1000),
  );

  // 导出相关状态
  type ExportFormat = "flvx" | "ny";
  const [exportModalOpen, setExportModalOpen] = useState(false);
  const [exportData, setExportData] = useState("");
  const [exportLoading, setExportLoading] = useState(false);
  const [exportFormat, setExportFormat] = useState<ExportFormat>("flvx");
  ...
  const [batchChangeTunnelModalOpen, setBatchChangeTunnelModalOpen] =
    useState(false);
  const [batchChangeStrategyModalOpen, setBatchChangeStrategyModalOpen] =
    useState(false);
  const [batchTargetTunnelId, setBatchTargetTunnelId] = useState<number | null>(
    null,
  );
  const [batchTargetStrategy, setBatchTargetStrategy] = useState<string | null>(
    null,
  );
  ...
  const loadForwardEntryHealth = useCallback(async (items: Forward[], force = false) => {
    const ids = items
      .map((item) => item.id)
      .filter((id): id is number => typeof id === "number" && id > 0);

    if (ids.length === 0) {
      setForwardEntryHealthMap({});
      setForwardEntryHealthStateMap({});
      setEntryHealthRequestInFlight(false);
      setLastEntryHealthCheckedAt(null);
      setEntryHealthCountdownSec(
        Math.ceil(ENTRY_HEALTH_REFRESH_INTERVAL_MS / 1000),
      );
      return;
    }

    setEntryHealthRequestInFlight(true);
    setForwardEntryHealthStateMap((prev) => {
      const next: Record<number, ForwardEntryHealthFetchState> = {};

      ids.forEach((id) => {
        next[id] = prev[id] === "ready" ? "ready" : "loading";
      });
    ...
    try {
      const response = await getForwardEntryStatusBatch(ids, force);

      if (response.code === 0 && Array.isArray(response.data)) {
        const nextMap: Record<number, ForwardEntryHealthSummary> = {};

        response.data.forEach((item) => {
          if (typeof item.forwardId !== "number" || item.forwardId <= 0) {
            return;
          }
          nextMap[item.forwardId] = {
            total: Number(item.total || 0),
            healthy: Number(item.healthy || 0),
            overallStatus:
        ...
        setForwardEntryHealthMap(nextMap);
        setForwardEntryHealthStateMap(() => {
          const next: Record<number, ForwardEntryHealthFetchState> = {};

          ids.forEach((id) => {
            next[id] = nextMap[id] ? "ready" : "failed";
          });
        ...
        setForwardEntryHealthStateMap((prev) => {
          ...
          ids.forEach((id) => {
            next[id] = prev[id] === "ready" ? "ready" : "failed";
          });
      ...
      setEntryHealthRequestInFlight(false);
      setLastEntryHealthCheckedAt(Date.now());
      setEntryHealthCountdownSec(
        Math.ceil(ENTRY_HEALTH_REFRESH_INTERVAL_MS / 1000),
      );
    }
  ...
  const loadForwardTargetHealth = useCallback(async (items: Forward[], force = false) => {
    const ids = items
      .map((item) => item.id)
      .filter((id): id is number => typeof id === "number" && id > 0);

    if (ids.length === 0) {
      setForwardTargetHealthMap({});
      setForwardTargetHealthStateMap({});
      return;
    }

    setForwardTargetHealthStateMap((prev) => {
      const next: Record<number, ForwardEntryHealthFetchState> = {};
    ...
    try {
      const response = await getForwardTargetStatusBatch(ids, force);

      if (response.code === 0 && Array.isArray(response.data)) {
        const nextMap: Record<number, ForwardTargetHealthSummary> = {};
        ...
        response.data.forEach((item) => {
          ...
          if (
            !force &&
            (item.overallStatus === "partial" || item.overallStatus === "failed")
          ) {
            suspiciousIds.push(item.forwardId);
          }
        });

        setForwardTargetHealthMap(nextMap);
        setForwardTargetHealthStateMap(() => {
          const next: Record<number, ForwardEntryHealthFetchState> = {};
        ...
        if (suspiciousIds.length > 0) {
          void (async () => {
            try {
              const refreshResponse = await getForwardTargetStatusBatch(
                suspiciousIds,
                true,
              );
              ...
              const refreshedMap: Record<number, ForwardTargetHealthSummary> =
                {};
              ...
              if (Object.keys(refreshedMap).length === 0) {
                return;
              }

              setForwardTargetHealthMap((prev) => ({
                ...prev,
                ...refreshedMap,
              }));
              setForwardTargetHealthStateMap((prev) => {
                ...
                suspiciousIds.forEach((id) => {
                  next[id] = refreshedMap[id] ? "ready" : next[id] || "failed";
                });

                return next;
              });
            } catch {
              // Keep the first response if the follow-up revalidation fails.
            }
          })();
        }
      } else {
  ...
  const loadForwardSLAStatus = useCallback(async (items: Forward[]) => {
    const ids = items
      .map((item) => item.id)
      .filter((id): id is number => typeof id === "number" && id > 0);

    if (ids.length === 0) {
      setForwardSLAStatusMap({});
      return;
    }

    try {
      const response = await getForwardSLAStatusBatch(ids);

      if (response.code !== 0 || !Array.isArray(response.data)) {
        return;
      }

      const nextMap: Record<number, ForwardSLAStatusSummaryApiItem> = {};

      response.data.forEach((item) => {
        if (typeof item.forwardId !== "number" || item.forwardId <= 0) {
          return;
        }
        nextMap[item.forwardId] = item;
      });

      setForwardSLAStatusMap(nextMap);
    } catch {
      // Keep the last SLA snapshot if the refresh fails.
    }
  }, []);

  const handleRefreshEntryHealth = useCallback(() => {
    if (entryHealthRequestInFlight || forwards.length === 0) {
      return;
    }
    void loadForwardEntryHealth(forwards, true);
  }, [entryHealthRequestInFlight, forwards, loadForwardEntryHealth]);
  ...
  const applyForwardList = useCallback(
    async (items: Forward[]) => {
      ...
      void loadForwardEntryHealth(mergedForwards);
      void loadForwardTargetHealth(mergedForwards);
      void loadForwardSLAStatus(mergedForwards);
    ...
    [
      loadForwardEntryHealth,
      loadForwardSLAStatus,
      loadForwardTargetHealth,
  ...
  const validateForm = (): boolean => {
    ...
    if (form.mode === "sni") {
      const rules = parseSniRulesInput(form.sniRules);

      if (!form.sniRules.trim()) {
        newErrors.sniRules = "请输入 SNI 分流规则";
      } else if (
        rules.length === 0 ||
        rules.some((rule) => !isValidSniRuleHost(rule.host))
      ) {
        newErrors.sniRules =
          "每行必须是合法域名，例如 hk1.example.com 或 *.example.com";
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const processedRemoteAddr = form.remoteAddr
        ...
        .filter((addr) => addr)
        .join(",");
      const selectorCount = getForwardRuleCount(form);
      ...
      if (isEdit) {
        const updateData = {
          ...
          sniRules: form.mode === "sni" ? form.sniRules : "",
          strategy: selectorCount > 1 ? form.strategy : "fifo",
      ...
      if (res.code === 0) {
        ...
        if (form.mode === "sni") {
          toast.success(
            `SNI 分流已下发到入口端口 ${form.inPort || 443}，共 ${selectorCount} 条规则`,
          );
        }
        setModalOpen(false);
        await refreshForwardList(false);
        ...
        toast.error(
          form.mode === "sni"
            ? `SNI 分流部署失败：${res.msg || "操作失败"}`
            : res.msg || "操作失败",
        );
      }
    } catch {
      toast.error(form.mode === "sni" ? "SNI 分流部署失败" : "操作失败");
    } finally {
  ...
  // 显示地址列表弹窗
  const showForwardTargetModal = (forward: Forward) => {
    if (forward.mode !== "sni") {
    ...
    const targetSummary = formatRemoteAddress(forward.remoteAddr);
    setAddressList(
      rules.map((rule, index) => ({
        id: index,
        address: `${rule.host} -> ${targetSummary}`,
        copying: false,
      })),
    );
    setAddressModalTitle(`SNI 规则 (${rules.length})`);
  ...
  const showAddressModal = (
    ...
    if (
      ...
      void (async () => {
        try {
          const response = await getForwardEntryStatus(forwardId);

          if (response.code === 0 && Array.isArray(response.data)) {
            const statusMap = new Map(
              response.data.map((item) => [item.address, item]),
            );

            setAddressList((prev) =>
              prev.map((item) => {
                const status = statusMap.get(item.address);
                ...
                return {
                  ...
                  occupiedByExternal: Boolean(status.occupiedByExternal),
  ...
  useEffect(() => {
    ...
    const refreshIfVisible = () => {
      ...
      void loadForwardEntryHealth(forwards);
    };

    const timer = window.setInterval(refreshIfVisible, ENTRY_HEALTH_REFRESH_INTERVAL_MS);
  ...
  }, [forwards, loadForwardEntryHealth]);
  ...
  useEffect(() => {
    ...
    const refreshIfVisible = () => {
      ...
      void loadForwardTargetHealth(forwards);
    };
  ...
  }, [forwards, loadForwardTargetHealth]);
  ...
  useEffect(() => {
    ...
    const refreshIfVisible = () => {
      ...
      void loadForwardSLAStatus(forwards);
    };
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      if (entryHealthRequestInFlight) {
        setEntryHealthCountdownSec(0);
        return;
      }

      if (!lastEntryHealthCheckedAt) {
        setEntryHealthCountdownSec(
          Math.ceil(ENTRY_HEALTH_REFRESH_INTERVAL_MS / 1000),
        );
        return;
      ...
      const elapsed = Date.now() - lastEntryHealthCheckedAt;
      const remaining = Math.max(
        0,
        Math.ceil((ENTRY_HEALTH_REFRESH_INTERVAL_MS - elapsed) / 1000),
      );

      setEntryHealthCountdownSec(remaining);
    };
  ...
  }, [forwards.length, entryHealthRequestInFlight, lastEntryHealthCheckedAt]);
  ...
  const executeExport = () => {
    ...
    try {
      ...
      if (exportFormat === "ny") {
        const result = formatForwardsAsNyData(forwardsToExport);

        if (result.lines.length === 0) {
          setExportWarnings(
            result.skipped.map(
              (item) => `规则「${item.name}」已跳过：${item.reason}`,
            ),
          );
          toast.error("所选隧道没有可导出的 Nyanpass 规则");
          setExportLoading(false);

          return;
        }

        setExportData(result.lines.join("\n"));
        setExportWarnings(
          result.skipped.map(
            (item) => `规则「${item.name}」已跳过：${item.reason}`,
          ),
        ...
        if (result.skipped.length > 0) {
          toast(
            `已生成 Nyanpass 导出，另有 ${result.skipped.length} 条规则因格式不兼容被跳过`,
          );
        }
      } else {
  ...
  const copyExportData = async () => {
    await copyToClipboard(
      exportData,
      exportFormat === "ny" ? "Nyanpass 规则数据" : "规则数据",
    );
  };
  ...
  const getEntryHealthDisplay = (forwardId: number) => {
    const summary = forwardEntryHealthMap[forwardId];
    const fetchState = forwardEntryHealthStateMap[forwardId] || "idle";

    if (summary?.overallStatus === "unknown") {
      return {
        color: "default" as const,
        text: "入口待人工确认",
        total: summary.total,
        healthy: summary.healthy,
        unhealthy: Math.max(summary.total - summary.healthy, 0),
        detail: "节点暂未提供完整部署摘要，可点“立即刷新一次”或点开查看即时结果",
      };
    }
    ...
    (summary.items || []).forEach((item) => {
      ...
      const reason = (item.reason || "").trim() || "状态未知";
      reasonMap.set(reason, (reasonMap.get(reason) || 0) + 1);
    });
    const detail = Array.from(reasonMap.entries())
      .slice(0, 2)
      .map(([reason, count]) => (count > 1 ? `${reason} x${count}` : reason))
      .join("；");

    const unhealthy = Math.max(summary.total - summary.healthy, 0);
    ...
    if (summary.healthy === summary.total) {
      return {
        color: "success" as const,
        text: `入口 ${summary.healthy}/${summary.total} 正常`,
        total: summary.total,
        healthy: summary.healthy,
        unhealthy,
        detail: "",
    ...
    if (summary.healthy > 0) {
      return {
        color: "warning" as const,
        text: `入口 正常 ${summary.healthy} / 异常 ${unhealthy}`,
        total: summary.total,
        healthy: summary.healthy,
        unhealthy,
        detail: detail ? `异常摘要：${detail}` : "",
      };
    }

    return {
      color: "danger" as const,
      text: `入口 ${summary.total} 个全部异常`,
      total: summary.total,
      healthy: summary.healthy,
      unhealthy,
      detail: detail ? `异常摘要：${detail}` : "",
  ...
  // 获取地址数量
  // 处理拖拽结束
  const getTargetHealthDisplay = (forwardId: number) => {
    const summary = forwardTargetHealthMap[forwardId];
    const fetchState = forwardTargetHealthStateMap[forwardId] || "idle";
    ...
    (summary.items || []).forEach((item) => {
      ...
      const reason = (item.reason || "").trim() || "未知异常";
      reasonMap.set(reason, (reasonMap.get(reason) || 0) + 1);
    });
    const detail = Array.from(reasonMap.entries())
      .slice(0, 2)
    ...
    if (summary.healthy === summary.total) {
      return {
        color: "success" as const,
        text: `落地 ${summary.healthy}/${summary.total} 正常`,
        total: summary.total,
        healthy: summary.healthy,
        unhealthy,
        detail: "",
    ...
    if (summary.healthy > 0) {
      return {
        color: "warning" as const,
        text: `落地 正常 ${summary.healthy} / 异常 ${unhealthy}`,
        total: summary.total,
        healthy: summary.healthy,
        unhealthy,
        detail,
    ...
    return {
      color: "danger" as const,
      text: `落地异常 ${unhealthy}/${summary.total}`,
      total: summary.total,
      healthy: summary.healthy,
      unhealthy,
      detail,
  ...
  const getSLADisplay = (forwardId: number) => {
    const summary = forwardSLAStatusMap[forwardId];

    if (!summary || !summary.checkedAt) {
      return {
        color: "default" as const,
        text: "SLA 采集中",
        detail: "等待入口和落地首轮探测",
    ...
    const clampedUptime = Math.max(
      0,
      Math.min(1, Number(summary.uptime24h) || 0),
    );
    const uptimePercent =
      summary.samples24h > 0
        ? `${Math.round(clampedUptime * 1000) / 10}%`
        : "--";
    const baseDetail = [
      `入口 ${summary.entryHealthy}/${summary.entryTotal}`,
      `落地 ${summary.targetHealthy}/${summary.targetTotal}`,
    ].join(" · ");
    ...
    switch (summary.overallStatus) {
      case "healthy":
        return {
          color: "success" as const,
          text: `SLA ${uptimePercent}`,
          detail: baseDetail,
        };
      case "partial":
        return {
          color: "warning" as const,
          text: `SLA 降级 ${uptimePercent}`,
          detail: summary.reason || baseDetail,
        };
      case "failed":
        return {
          color: "danger" as const,
          text: `SLA 异常 ${uptimePercent}`,
          detail: summary.reason || baseDetail,
        };
      default:
        return {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const activeMode = activeForward?.mode ?? "direct";
    const overMode = overForward?.mode ?? "direct";
  ...
  const setIdsSelected = useCallback((ids: number[], isSelected: boolean) => {
  ...
  const getSelectionStateForIds = useCallback(
    (ids: number[]) => {
      const normalizedIds = Array.from(new Set(ids.filter((id) => id > 0)));
      const selectedCount = normalizedIds.reduce(
      ...
      return {
        ...
        selectedCount,
        isAllSelected:
          normalizedIds.length > 0 && selectedCount === normalizedIds.length,
        isIndeterminate:
          selectedCount > 0 && selectedCount < normalizedIds.length,
      };
    },
    [selectedIds],
  );
  ...
  const handleBatchDelete = async () => {
    ...
    void (async () => {
      try {
        const outcome = await executeForwardBatchDelete(
          ids,
          (job) => {
            const total = Math.max(1, Number(job.total) || ids.length || 1);
  ...
  const handleBatchToggleService = async (enable: boolean) => {
    ...
    setBatchProgress({
      active: true,
      label: `已提交后台${enable ? "启用" : "停用"}任务，正在启动...`,
      percent: 30,
    });
    setSelectedIds(new Set());
    setSelectMode(false);
      ...
      try {
        const outcome = await executeForwardBatchToggleService(
          ...
          (job) => {
            ...
            setBatchProgress({
              active: true,
              label: `后台${enable ? "启用" : "停用"}中... ${completed}/${total}`,
              percent,
            });
          },
        );
      ...
      try {
        const outcome = await executeForwardBatchRedeploy(
          ...
          (job) => {
            ...
            setBatchProgress({
              active: true,
              label: `后台同步到节点中... ${completed}/${total}`,
              percent,
            });
          },
        );
  ...
  const handleBatchChangeStrategy = async () => {
    if (selectedIds.size === 0 || !batchTargetStrategy) return;
    const ids = Array.from(selectedIds);
    setBatchLoading(true);
    setBatchProgress({
      active: true,
      label: `已提交后台改策略任务，正在启动...`,
      percent: 30,
    });
    setSelectedIds(new Set());
    setSelectMode(false);
    setBatchChangeStrategyModalOpen(false);
    void (async () => {
      try {
        const outcome = await executeForwardBatchUpdateStrategy(
          ids,
          batchTargetStrategy,
          (job) => {
            const total = Math.max(1, Number(job.total) || ids.length || 1);
        ...
        if (outcome.shouldRefresh) {
          ...
          if (outcome.resetTargetStrategy) {
            setBatchTargetStrategy(null);
          }
          await refreshForwardList(false);
        }
      } finally {
  ...
  const orderedForwards = useMemo((): Forward[] => {
    ...
    if (searchParams.remoteAddr.trim()) {
      ...
      filteredForwards = filteredForwards.filter(
        (f) =>
          getForwardTargetSearchText(f).toLowerCase().includes(lowerAddr),
      );
    }
  ...
  const sortedForwardSelectionState = useMemo(
    () => getSelectionStateForIds(sortedForwards.map((forward) => forward.id)),
    [getSelectionStateForIds, sortedForwards],
  );
  const selectAll = useCallback(() => {
    setIdsSelected(
      sortedForwards.map((forward) => forward.id),
      true,
    );
  }, [setIdsSelected, sortedForwards]);
  const isAllSelected = sortedForwardSelectionState.isAllSelected;
  const isIndeterminate = sortedForwardSelectionState.isIndeterminate;
  ...
  const renderForwardCard = (forward: Forward, listeners?: any) => {
    ...
    return (
      <Card
        key={forward.id}
        className={`group h-full flex flex-col shadow-sm border hover:shadow-md transition-shadow duration-200 overflow-hidden ${
          entryHealthDisplay.color === "danger"
            ? "border-danger-200/80 dark:border-danger-500/40"
            : entryHealthDisplay.color === "warning"
              ? "border-warning-200/80 dark:border-warning-500/40"
              : "border-divider"
        }`}
      >
        ...
        <CardBody className="flex flex-1 flex-col pt-0 pb-3 md:pt-0 md:pb-3">
          <div className="space-y-2 flex-1">
            ...
            <div className="space-y-1">
              <button
                className={`cursor-pointer px-2 py-1 rounded border transition-colors duration-200 ${entryPanelClassName} ${
                  hasMultipleAddresses(forward.inIp)
                    ? ""
                    : ""
                }`}
              ...
              <button
                ...
                type="button"
                onClick={() =>
                  showForwardTargetModal(forward)
                }
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
  ...
  const hasForwardEntryPolling = forwards.length > 0;
  const lastEntryHealthCheckedLabel = lastEntryHealthCheckedAt
    ? new Date(lastEntryHealthCheckedAt).toLocaleTimeString()
    : "--:--:--";
  const lastEntryHealthRelativeLabel =
    formatRelativeTimeLabel(lastEntryHealthCheckedAt);
  ...
  return (
    <AnimatedPage className="px-3 lg:px-6 py-8">
            ...
            {selectedIds.size > 0 ? (
              <>
                ...
                <Button
                  ...
                  size="sm"
                  variant="flat"
                  onPress={() => setBatchChangeStrategyModalOpen(true)}
                >
                  策略
                </Button>
                <Button
      ...
      {hasForwardEntryPolling && (
        <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-default-200 bg-default-50/80 px-4 py-3 text-sm text-default-600">
          <Chip color="primary" size="sm" variant="flat">
            列表每 60 秒读取一次入口部署摘要
          </Chip>
          <Chip
            color={entryHealthRequestInFlight ? "warning" : "default"}
            size="sm"
            variant="flat"
          >
            {entryHealthRequestInFlight
              ? "本轮摘要刷新中"
              : `${entryHealthCountdownSec} 秒后自动重试`}
          </Chip>
          <span className="text-xs text-default-500">
            最近刷新时间：{lastEntryHealthCheckedLabel} ({lastEntryHealthRelativeLabel})
          </span>
          <span className="text-xs text-default-400">
            列表仅显示缓存摘要；点“立即刷新一次”会重新生成，点开入口可查看即时连通性明细
          </span>
          <Button
            color="primary"
            isDisabled={entryHealthRequestInFlight}
            size="sm"
            variant="flat"
            onPress={handleRefreshEntryHealth}
          >
            立即刷新并更新摘要
          </Button>
        </div>
                  ...
                  >
                    ...
                    >
                      <TableHeader>
                        ...
                        <TableColumn className="w-[120px]">状态</TableColumn>
                      ...
                      >
                        {(forward) => (
                          <SortableCompactTableRow
                            ...
                            showForwardTargetModal={showForwardTargetModal}
                    ...
                    >
                      ...
                      >
                        {group.tunnels.map((tunnel) => {
                          const modeGroups = buildForwardModeGroups(tunnel.items);
                          const collapsed =
                            sanitizedCollapsedTunnelGroups[
                              ...
                              >
                                <div className="space-y-3">
                                  {modeGroups.map((modeGroup) => {
                                    const modeSortableForwardIds =
                                      modeGroup.items
                                        .map((item) => item.id)
                                        .filter((id) => id > 0);
                                    const modeSelectionState =
                                      getSelectionStateForIds(
                                        modeSortableForwardIds,
                                      );

                                    return (
                                      <div
                                        key={`grouped-table-${group.userId}-${tunnel.tunnelKey}-${modeGroup.mode}`}
                                        className="overflow-hidden rounded-lg border border-divider bg-content1/60"
                                      >
                                        <div className="flex items-center justify-between border-b border-divider bg-default-100/50 px-4 py-2.5">
                                          <div className="flex min-w-0 items-center gap-2">
                                            <Chip
                                              color={
                                                getForwardModeGroupChipColor(
                                                  modeGroup.mode,
                                                ) as any
                                              }
                                              size="sm"
                                              variant="flat"
                                            >
                                              {getForwardModeGroupLabel(
                                                modeGroup.mode,
                                              )}
                                            </Chip>
                                            <span className="truncate text-xs text-default-500">
                                              {getForwardModeGroupDescription(
                                                modeGroup.mode,
                                              )}
                                            </span>
                                          </div>
                                          <span className="text-xs text-default-600">
                                            {modeGroup.items.length} 条
                                          </span>
                                        </div>

                                        <SortableContext
                                          items={modeSortableForwardIds}
                                          strategy={verticalListSortingStrategy}
                                        >
                                          <Table
                                            aria-label={`${group.userName}-${tunnel.tunnelName}-${getForwardModeGroupLabel(modeGroup.mode)}规则列表`}
                                            ...
                                            <TableHeader>
                                              {true && (
                                                <TableColumn className="w-14">
                                                  {/* @ts-ignore */}
                                                  <Checkbox
                                                    aria-label={`${getForwardModeGroupLabel(modeGroup.mode)}全选`}
                                                    isIndeterminate={
                                                      modeSelectionState.isIndeterminate
                                                    }
                                                    isSelected={
                                                      modeSelectionState.isAllSelected
                                                    }
                                                    onValueChange={(
                                                      isSelected,
                                                    ) =>
                            ...
                            >
                              <div className="space-y-3">
                                {modeGroups.map((modeGroup) => {
                                  ...
                                  return (
                                    <div
                                      key={`direct-group-${group.userId}-${tunnel.tunnelKey}-${modeGroup.mode}`}
                                      className="space-y-3"
                                    >
                                      <div className="flex items-center justify-between rounded-lg border border-divider bg-default-100/50 px-3 py-2">
                                        <div className="flex min-w-0 items-center gap-2">
                                          <Chip
                                            color={
                                              getForwardModeGroupChipColor(
            ...
            <>
              <ModalHeader className="flex flex-col gap-1">
                <h2 className="text-xl font-bold">
                  {isEdit ? "编辑转发规则" : "新增转发规则"}
                </h2>
                <p className="text-small text-default-500">
                  {isEdit
                    ? "规则运行在既有隧道之上。这里调整的是转发规则，不是隧道本身。"
                    : "先选择基础隧道，再决定用直连转发或 SNI 分流。SNI 是规则模式，不会替代隧道配置。"}
                </p>
              </ModalHeader>
              <ModalBody>
                <div className="space-y-4 pb-4">
                  <div className="rounded-xl border border-primary/15 bg-primary/5 px-4 py-3 text-sm text-default-700">
                    <p className="font-medium text-foreground">
                      说明
                    </p>
                    <p className="mt-1">
                      隧道负责线路与节点链路，规则负责这条流量如何转发。
                    </p>
                    <p className="mt-1">
                      如果要新增或修改入口/出口节点，请前往「隧道」页面；这里仅配置挂在该隧道上的转发规则。
                    </p>
                  </div>
                  ...
                  <Select
                    description={
                      isEdit
                        ? "更改基础隧道会释放原端口，并在新隧道上重新分配或复用入口端口"
                        : "规则始终依附在基础隧道上运行；节点与链路请在「隧道」页面管理"
                    }
                    errorMessage={errors.tunnelId}
                    isInvalid={!!errors.tunnelId}
                    label="基础隧道"
                    placeholder="请选择承载这条规则的基础隧道"
                    selectedKeys={
                  ...
                  <Select
                    description="直连模式是普通端口转发；SNI 模式是在已选隧道上做共享端口分流，默认建议使用 443。"
                    label="转发模式"
                    selectedKeys={[form.mode]}
                    variant="bordered"
                    onSelectionChange={(keys) => {
                      const selectedKey =
                        (Array.from(keys)[0] as "direct" | "sni") || "direct";

                      setForm((prev) => ({
                        ...prev,
                        mode: selectedKey,
                        inPort:
                          selectedKey === "sni" && prev.inPort === null
                            ? 443
                            : prev.inPort,
                      }));
                    }}
                  >
                    <SelectItem key="direct">直连转发</SelectItem>
                    <SelectItem key="sni">SNI 分流 (共享端口)</SelectItem>
                  </Select>

                  <Input
                    description={
                      form.mode === "sni"
                        ? "SNI 模式默认建议 443，也可填写 8443 等 TLS 端口做验收或特殊部署"
                        : currentTunnelPortRange
                    ...
                    label="入口端口"
                    placeholder={form.mode === "sni" ? "默认 443，可改成 8443 做测试" : "留空则自动分配可用端口"}
                    type="number"
                  ...
                  {form.mode === "sni" && (
                    <Textarea
                      description="每行输入一个 SNI 域名。将匹配这些域名的 TLS 流量转发至设置的落地地址。支持泛域名(如 *.example.com)。"
                      errorMessage={errors.sniRules}
                      isInvalid={!!errors.sniRules}
                      label="SNI 域名列表"
                      maxRows={8}
                      minRows={4}
                  ...
                  <Textarea
                    description={form.mode === "sni" ? "格式: IP:端口 或 域名:端口，匹配 SNI 后将转发至此处的落地地址" : "格式: IP:端口 或 域名:端口，支持多个地址（每行一个）"}
      ...
      >
        <ModalContent>
          ...
          <ModalBody className="pb-6">
            ...
            {addressStatusLoading && addressModalTitle.includes("入口") ? (
              <div className="mb-3 text-sm text-default-500">正在检查入口状态...</div>
            ) : null}
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {addressList.map((item) => (
                <div
                  key={item.id}
                  className="p-3 border border-default-200 dark:border-default-100 rounded-lg"
                >
                  ...
                  {addressModalTitle.includes("入口") ? (
                    <div className="mt-2 flex items-center gap-2 text-xs">
                      <Chip
                        color={item.healthy ? "success" : "warning"}
                        size="sm"
                        variant="flat"
                      >
                        {item.healthy
                      ...
                      <Chip
                        color={item.reachable ? "primary" : "default"}
                        size="sm"
                        variant="flat"
                      >
                        {item.reachable ? "可连通" : "未连通"}
                      </Chip>
                      {item.occupiedByExternal ? (
                  ...
                  {addressModalTitle.includes("入口") && item.reason ? (
                    <div className="mt-1 text-xs text-default-500 break-all">
                      {item.reason}
                    </div>
                  ) : null}
                </div>
        ...
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1">
            ...
            {exportFormat === "ny" ? (
              <>
                <p className="text-small text-default-500">
                  Nyanpass 格式：每行一个 JSON 对象，包含
                  `dest`、`dest_policy`、`listen_port`、`name`
                </p>
                <p className="text-small text-default-400">
                  兼容优先：`dest_policy` 固定导出为 `least_load`，且仅导出
                  direct 规则
                </p>
              </>
            ) : (
            ...
            <div className="space-y-4">
              <div>
                <Select
                  ...
                  variant="bordered"
                  onSelectionChange={(keys) => {
                    const selectedKey = Array.from(keys)[0] as ExportFormat;

                    if (selectedKey) {
                      setExportFormat(selectedKey);
                      setExportData("");
                      setExportWarnings([]);
                    }
                  }}
                >
                  ...
                  <SelectItem key="ny" textValue="Nyanpass格式">
                    Nyanpass 格式（JSON Lines）
                  </SelectItem>
                </Select>
              </div>
              ...
              {exportWarnings.length > 0 ? (
                <div className="rounded-lg border border-warning-200 bg-warning-50 p-3 dark:border-warning-400/30 dark:bg-warning-500/10">
                  <div className="text-sm font-medium text-warning-700 dark:text-warning-300">
                    已跳过 {exportWarnings.length} 条不兼容规则
                  </div>
                  <div className="mt-2 max-h-28 space-y-1 overflow-y-auto">
                    {exportWarnings.map((warning, index) => (
                      <p
                        key={`${index}-${warning}`}
                        className="text-xs text-warning-700 dark:text-warning-200"
                      >
                        {warning}
                      </p>
                    ))}
      ...
      <Modal
        ...
        isOpen={batchChangeStrategyModalOpen}
        onOpenChange={setBatchChangeStrategyModalOpen}
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>策略</ModalHeader>
              <ModalBody>
                <p className="mb-4">
                  将选中的 {selectedIds.size} 项规则统一改为新策略：
                </p>
                <Select
                  label="目标策略"
                  placeholder="请选择负载均衡策略"
                  selectedKeys={batchTargetStrategy ? [batchTargetStrategy] : []}
                  onSelectionChange={(keys) => {
                    const selected = Array.from(keys)[0];

                    setBatchTargetStrategy(
              ...
              <ModalFooter>
                ...
                <Button
                  color="primary"
                  isDisabled={!batchTargetStrategy}
                  isLoading={batchLoading}
                  onPress={handleBatchChangeStrategy}
                >
                  确认改策略
                </Button>
              </ModalFooter>
