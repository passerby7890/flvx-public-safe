...
type TunnelProtocolKey = "tls" | "wss" | "tcp" | "mtls" | "mwss" | "mtcp";

type TunnelProtocolMeta = {
  description: string;
  speed: string;
  stealth: string;
...
const TUNNEL_PROTOCOL_META: Record<TunnelProtocolKey, TunnelProtocolMeta> = {
  tls: {
    title: "TLS",
    speed: "高",
    stealth: "中",
    recommendation: "默认首选",
    description: "最稳的通用协议，兼顾速度、稳定性和加密，适合大多数生产场景。",
  },
  wss: {
    title: "WSS",
    speed: "中高",
    stealth: "高",
    recommendation: "公网复杂环境",
    description: "更像正常 HTTPS / WebSocket 流量，适合更重视流量形态伪装的场景。",
  },
  tcp: {
    title: "TCP",
    speed: "很高",
    stealth: "低",
    recommendation: "内网/可信网络",
    description: "开销最低、速度最快，但特征最明显，不建议作为公网默认协议。",
  },
  mtls: {
    title: "MTLS",
    speed: "高",
    stealth: "中高",
    recommendation: "高并发专线",
    description: "多路复用加 TLS，适合大量连接并发，同时保留较好的安全性。",
  },
  mwss: {
    title: "MWSS",
    speed: "中",
    stealth: "很高",
    recommendation: "高并发且要伪装",
    description: "多路复用加 WSS，适合高并发并且希望更像正常 Web 流量的场景。",
  },
  mtcp: {
    title: "MTCP",
    speed: "很高",
    stealth: "低",
    recommendation: "高并发内网",
    description: "多路复用加 TCP，适合追求极致开销和高并发，但不适合复杂公网环境。",
  },
};

const TunnelProtocolHint = ({
  protocol,
  className = "",
}: {
  protocol?: string;
  ...
  const key = ((protocol || "tls").trim().toLowerCase() ||
    "tls") as TunnelProtocolKey;
  const meta = TUNNEL_PROTOCOL_META[key] || TUNNEL_PROTOCOL_META.tls;

  return (
    <div
      className={`rounded-lg border border-primary/15 bg-primary/5 px-3 py-2 ${className}`.trim()}
    >
      ...
      <p className="mt-2 text-xs leading-5 text-default-600">{meta.description}</p>
    </div>
  );
};
...
const TUNNEL_UPDATE_JOB_POLL_INTERVAL_MS = 1500;
...
const waitForTunnelUpdateJob = async (
  jobId: string,
  onProgress: (message: string) => void,
) => {
  while (true) {
    const response = await getTunnelUpdateJob(jobId);

    if (response.code !== 0 || !response.data) {
      throw new Error(response.msg || "获取隧道同步任务状态失败");
    }

    const job = response.data;
    const total = Math.max(1, Number(job.total) || 1);
    const completed = Math.min(Number(job.completed) || 0, total);
    onProgress(`后台同步中... ${completed}/${total}`);
    ...
    await new Promise((resolve) => {
      window.setTimeout(resolve, TUNNEL_UPDATE_JOB_POLL_INTERVAL_MS);
    });
  }
};
      ...
      if (response.code === 0) {
        const responseData =
          response.data && typeof response.data === "object"
            ? (response.data as { jobId?: string })
            : null;
        const jobId =
          isEdit && responseData && typeof responseData.jobId === "string"
            ? responseData.jobId
            : "";
        ...
        if (isEdit && jobId) {
          const loadingToastId = toast.loading("后台同步中...");

          void (async () => {
            try {
              const job = await waitForTunnelUpdateJob(jobId, (message) => {
                toast.loading(message, { id: loadingToastId });
              });

              if ((job.failCount || 0) > 0) {
                const firstFailure = Array.isArray(job.failures)
                  ? job.failures[0]
                  : null;
                const reason =
                  firstFailure && typeof firstFailure.reason === "string"
                    ? firstFailure.reason
                    : "部分规则同步失败";
                toast.error(`后台同步完成，但存在失败: ${reason}`, {
                  id: loadingToastId,
                });
              } else {
                toast.success("后台同步完成", { id: loadingToastId });
              }
  ...
  return (
    <AnimatedPage className="px-3 lg:px-6 py-8">
      ...
      <Modal
        backdrop="blur"
        classNames={{
          base: "!w-[calc(100%-32px)] !mx-auto sm:!w-full rounded-2xl overflow-hidden max-h-[calc(100vh-32px)]",
          body: "overflow-y-auto",
          footer: "border-t border-divider bg-content1/95 backdrop-blur-sm",
        }}
        isOpen={modalOpen}
        placement="center"
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                                  ...
                                  <TunnelProtocolHint protocol={protocol} />
