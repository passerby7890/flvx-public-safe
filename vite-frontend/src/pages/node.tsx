...
const NODE_AUTO_REFRESH_INTERVAL_MS = 5 * 60 * 1000;
...
const getReleaseChannelLabel = (channel: ReleaseChannel): string => {
  switch (channel) {
    case "dev":
      return "测试版";
    case "patched":
...
const getReleaseChannelSummary = (channel: ReleaseChannel): string => {
  switch (channel) {
    case "dev":
      return "测试版（GitHub 预发布，含 alpha / beta / rc）";
    case "patched":
      return "当前面板内建版（与本面板保持同步）";
    default:
      return "正式版（GitHub 稳定版）";
  }
};
...
export default function NodePage() {
  ...
  const [upgradeSubmitLoading, setUpgradeSubmitLoading] = useState(false);
  ...
  const handleCopyInstallCommand = async (
    ...
    try {
      ...
      if (res.code === 0 && res.data) {
        ...
        if (copied) {
          toast.success(`${getReleaseChannelLabel(channel)}安装命令已复制到剪贴板`);
        } else {
  ...
  const openUpgradeModal = async (
    ...
    const defaultChannel: ReleaseChannel = "patched";
    ...
    try {
      if (upgradeTarget === "single" && upgradeTargetNodeId) {
        ...
        try {
          ...
          if (res.code === 0) {
            toast.success(
              `已向节点 ${node.name} 发送${getReleaseChannelLabel(releaseChannel)}升级命令，节点将自动重启`,
            );
            setUpgradeModalOpen(false);
          } else {
            toast.error(
              `节点 ${node.name} 升级失败：${res.msg || "请稍后重试"}`,
            );
          }
        } catch {
          toast.error(`节点 ${node.name} 升级失败：网络错误，请重试`);
        } finally {
          setNodeList((prev) =>
            prev.map((n) =>
              n.id === upgradeTargetNodeId
                ? { ...n, upgradeLoading: false }
                : n,
            ),
          );
        }
        ...
        try {
          ...
          if (res.code === 0) {
            toast.success(
              `已向 ${selectedLocalIds.length} 个节点发送${getReleaseChannelLabel(releaseChannel)}升级命令`,
            );
            setUpgradeModalOpen(false);
          } else {
          ...
          toast.error("批量升级失败：网络错误，请重试");
        } finally {
          setBatchUpgradeLoading(false);
        }
      }
    } finally {
      setUpgradeSubmitLoading(false);
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Alert
                  ...
                  color="primary"
                  description="在线本地节点在 agent 接入后会自动同步地址信息：节点上线时立即同步一次，并每 5 分钟自动刷新 IPv4 / IPv6 / 额外 IP。手动填写主要用于初始创建或离线节点。"
                />
                ...
                <div className="space-y-4">
                  <Select
                    ...
                    onSelectionChange={(keys) => {
                      ...
                      setInstallChannel(selected || "patched");
                    }}
                  >
                    <SelectItem key="patched" textValue="patched">
                      当前面板内建版（推荐，节点版本与本面板同步）
                    </SelectItem>
                    <SelectItem key="stable" textValue="stable">
                      正式版（GitHub 稳定版）
                    </SelectItem>
                    <SelectItem key="dev" textValue="dev">
                      测试版（GitHub 预发布，含 alpha / beta / rc）
                    </SelectItem>
                  </Select>
                </div>
              </ModalBody>
          ...
          {(onClose) => (
            <>
                ...
                {releasesLoading ? (
                  ...
                  <div className="space-y-4">
                    <Select
                      ...
                      onSelectionChange={(keys) => {
                        const selected =
                          (Array.from(keys)[0] as ReleaseChannel) || "patched";
                    ...
                    <p className="text-sm text-default-500">
                      {selectedVersion
                        ...
                        : `未选择版本，将自动使用${getReleaseChannelSummary(releaseChannel)}`}
                    </p>
                  </div>
                )}
              </ModalBody>
              <ModalFooter>
                ...
                <Button
                  color="warning"
                  isDisabled={releasesLoading || upgradeSubmitLoading}
                  isLoading={upgradeSubmitLoading}
