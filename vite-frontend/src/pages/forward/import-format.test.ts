...
test("formatForwardsAsNyData exports direct forwards as nyanpass json lines", () => {
  const result = formatForwardsAsNyData([
    {
      name: "  Forward Name  ",
      mode: "direct",
      remoteAddr: "1.1.1.1:1111,2.2.2.2:2222",
...
test("formatForwardsAsNyData skips sni forwards", () => {
  const result = formatForwardsAsNyData([
    {
      name: "SNI Forward",
      mode: "sni",
  ...
  assert.match(result.skipped[0]?.reason || "", /SNI/);
});

test("formatForwardsAsNyData skips forwards without valid listen port", () => {
  const result = formatForwardsAsNyData([
    {
      name: "No Port",
      mode: "direct",
  ...
  assert.equal(result.lines.length, 0);
  assert.equal(result.skipped.length, 1);
  assert.match(result.skipped[0]?.reason || "", /端口/);
});
