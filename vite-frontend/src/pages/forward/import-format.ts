...
export interface NyExportItem {
  dest: string[];
  dest_policy: "least_load";
  listen_port: number;
  name: string;
}

export interface NyExportForwardInput {
  mode?: string;
  name: string;
  remoteAddr: string;
  inPort: number | null;
...
export interface NyExportResult {
  lines: string[];
  skipped: Array<{
    name: string;
    reason: string;
...
export const formatForwardsAsNyData = (
  items: NyExportForwardInput[],
): NyExportResult => {
  const result: NyExportResult = {
    lines: [],
    skipped: [],
  };
  ...
  items.forEach((item) => {
    ...
    if ((item.mode || "").trim().toLowerCase() === "sni") {
      result.skipped.push({
        name,
        reason: "Nyanpass 格式不支持 SNI 规则导出",
      });
      return;
    }

    const normalizedDest = normalizeDestList(item.remoteAddr);
    if (!normalizedDest || normalizedDest.length === 0) {
      result.skipped.push({
        name,
        reason: "目标地址为空",
    ...
    const invalidDest = normalizedDest.find(
      (destItem) => !ADDRESS_PATTERN.test(destItem),
    );
    if (invalidDest) {
      result.skipped.push({
        name,
        reason: `目标地址格式不支持: ${invalidDest}`,
      });
      return;
    }

    if (!isValidListenPort(item.inPort)) {
      result.skipped.push({
        name,
        reason: "入口端口无效或为空",
      });
    ...
    const exportItem: NyExportItem = {
      dest: normalizedDest,
      dest_policy: "least_load",
      listen_port: item.inPort,
      name,
    };

    result.lines.push(JSON.stringify(exportItem));
  });
