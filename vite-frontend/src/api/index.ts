import type {
  ...
  AsyncJobApiData,
  ForwardEntryStatusSummaryApiItem,
  ForwardSLAStatusSummaryApiItem,
  ForwardTargetStatusSummaryApiItem,
...
export type ReleaseChannel = "stable" | "dev" | "patched";

const TUNNEL_MUTATION_TIMEOUT_MS = 10 * 60 * 1000;
const FORWARD_MUTATION_TIMEOUT_MS = 10 * 60 * 1000;
const FORWARD_BATCH_TIMEOUT_MS = 15 * 60 * 1000;
...
export const createTunnel = (data: TunnelMutationPayload) =>
  Network.post("/tunnel/create", data, {
    timeout: TUNNEL_MUTATION_TIMEOUT_MS,
  });
...
export const getCoverDomainProfiles = () =>
  Network.post<CoverDomainProfileApiItem[]>("/cover/profile/list");
export const getCoverDomainProfile = (id: number) =>
  Network.post<CoverDomainProfileApiItem>("/cover/profile/get", { id });
export const updateCoverDomainProfile = (data: CoverDomainProfileApiItem) =>
  Network.post<CoverDomainProfileApiItem>("/cover/profile/update", data, {
    timeout: TUNNEL_MUTATION_TIMEOUT_MS,
  });
export const deleteCoverDomainProfile = (id: number) =>
  Network.post("/cover/profile/delete", { id }, {
    timeout: TUNNEL_MUTATION_TIMEOUT_MS,
  });
export const getTunnelCoverSelections = () =>
  Network.post<TunnelCoverSelectionApiItem[]>("/cover/tunnel/list");
export const getTunnelCoverSelection = (tunnelId: number) =>
  Network.post<TunnelCoverSelectionApiItem>("/cover/tunnel/get", { tunnelId });
export const updateTunnelCoverSelection = (data: TunnelCoverSelectionApiItem) =>
  Network.post("/cover/tunnel/update", data, {
    timeout: TUNNEL_MUTATION_TIMEOUT_MS,
  });
export const getNodeCoverServices = () =>
  Network.post<NodeCoverServiceApiItem[]>("/cover/node/list");
export const getNodeCoverService = (nodeId: number) =>
  Network.post<NodeCoverServiceApiItem>("/cover/node/get", { nodeId });
export const updateNodeCoverService = (data: NodeCoverServiceApiItem) =>
  Network.post("/cover/node/update", data, {
    timeout: TUNNEL_MUTATION_TIMEOUT_MS,
  });
export const syncNodeCoverService = (nodeId: number) =>
  Network.post<Record<string, unknown>>(
    "/cover/node/sync",
    { nodeId },
    { timeout: TUNNEL_MUTATION_TIMEOUT_MS },
  );
...
export const createForward = (data: ForwardMutationPayload) =>
  Network.post("/forward/create", data, {
    timeout: FORWARD_MUTATION_TIMEOUT_MS,
  });
...
export const deleteForward = (id: number) =>
  Network.post("/forward/delete", { id }, { timeout: FORWARD_MUTATION_TIMEOUT_MS });
export const forceDeleteForward = (id: number) =>
  Network.post("/forward/force-delete", { id }, { timeout: FORWARD_MUTATION_TIMEOUT_MS });
...
export const batchDeleteForwards = (ids: number[]) =>
  Network.post<BatchOperationResult>("/forward/batch-delete", { ids }, {
    timeout: FORWARD_BATCH_TIMEOUT_MS,
  });
...
export const batchUpdateForwardStrategy = (data: {
  ids: number[];
  strategy: string;
}) =>
  Network.post<BatchOperationResult>("/forward/batch-update-strategy", data, {
    timeout: FORWARD_BATCH_TIMEOUT_MS,
  });
export const startForwardBatchJob = (data: {
  action:
    | "delete"
    | "pause"
    | "resume"
  ...
  Network.post<ForwardBatchJobApiData>("/forward/batch-job/start", data, {
    timeout: FORWARD_MUTATION_TIMEOUT_MS,
  });
export const getForwardBatchJob = (jobId: string) =>
  Network.post<ForwardBatchJobApiData>("/forward/batch-job/get", { jobId }, {
    timeout: FORWARD_MUTATION_TIMEOUT_MS,
  });
export const getTunnelUpdateJob = (jobId: string) =>
  Network.post<AsyncJobApiData>("/tunnel/update-job/get", { jobId }, {
    timeout: FORWARD_MUTATION_TIMEOUT_MS,
  });
export const getForwardEntryStatus = (id: number) =>
  Network.post<
    Array<{
      nodeId: number;
      nodeName: string;
  ...
  >("/forward/entry-status", { id }, {
    timeout: FORWARD_MUTATION_TIMEOUT_MS,
  });
export const getForwardEntryStatusBatch = (ids: number[], force = false) =>
  Network.post<ForwardEntryStatusSummaryApiItem[]>(
    "/forward/entry-status/batch",
    { ids, force },
    {
      timeout: FORWARD_BATCH_TIMEOUT_MS,
    },
...
export const getForwardTargetStatusBatch = (ids: number[], force = false) =>
  Network.post<ForwardTargetStatusSummaryApiItem[]>(
    "/forward/target-status/batch",
    { ids, force },
    {
      timeout: FORWARD_BATCH_TIMEOUT_MS,
    },
...
export const getForwardSLAStatusBatch = (ids: number[]) =>
  Network.post<ForwardSLAStatusSummaryApiItem[]>(
    "/forward/sla-status/batch",
    { ids },
    {
      timeout: FORWARD_BATCH_TIMEOUT_MS,
    },
