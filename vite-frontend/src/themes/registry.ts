...
export function resolveSystemMode(): "light" | "dark" {
  ...
  const responsiveWidth = Math.min(
    Number(window.innerWidth || Number.MAX_SAFE_INTEGER),
    Number(window.visualViewport?.width || Number.MAX_SAFE_INTEGER),
    Number(window.screen?.width || Number.MAX_SAFE_INTEGER),
  );
  const isCompactTouchDevice =
    Number(navigator.maxTouchPoints || 0) > 1 &&
    Math.min(
      Number(window.screen?.width || responsiveWidth),
      Number(window.screen?.height || responsiveWidth),
    ) <= 1024;
  ...
  // Phones should default to the light palette unless the user explicitly
  // selected dark mode. This keeps the mobile UI visually aligned with the
  // desktop panel instead of inheriting the browser/system dark preference.
  if (
    Number.isFinite(responsiveWidth) &&
    responsiveWidth <= 768
  ) {
    return "light";
  }
  if (isCompactTouchDevice || isMobileBrowser) {
    return "light";
  }
