...
type Handler struct {
	...
	forwardEntrySummaryMu       sync.RWMutex
	forwardEntrySummaryCache    map[int64]forwardEntryStatusSummary
	forwardEntrySummaryChecked  map[int64]int64
	forwardEntrySummaryInflight map[int64]struct{}
	forwardEntrySummaryQueue    chan int64

	forwardTargetSummaryMu       sync.RWMutex
	forwardTargetSummaryCache    map[int64]forwardTargetStatusSummary
	forwardTargetSummaryChecked  map[int64]int64
	forwardTargetSummaryInflight map[int64]struct{}
	forwardTargetSummaryQueue    chan int64

	qualityProber *tunnelQualityProber
	batchJobs     *batchJobManager
}
...
func New(repo *repo.Repository, jwtSecret string) *Handler {
	h := &Handler{
		...
		forwardEntrySummaryCache:     make(map[int64]forwardEntryStatusSummary),
		forwardEntrySummaryChecked:   make(map[int64]int64),
		forwardEntrySummaryInflight:  make(map[int64]struct{}),
		forwardEntrySummaryQueue:     make(chan int64, 512),
		forwardTargetSummaryCache:    make(map[int64]forwardTargetStatusSummary),
		forwardTargetSummaryChecked:  make(map[int64]int64),
		forwardTargetSummaryInflight: make(map[int64]struct{}),
		forwardTargetSummaryQueue:    make(chan int64, 512),
		batchJobs:                    newBatchJobManager(),
	}
	...
	h.startForwardEntrySummaryWorkers()
	h.startForwardEntrySummaryRefresher()
	h.startForwardTargetSummaryWorkers()
	h.startForwardTargetSummaryRefresher()
	return h
}
...
func (h *Handler) Register(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/v1/node/custom-upgrade", h.nodeCustomUpgrade)
	mux.HandleFunc("/api/v1/node/custom-command", h.nodeCustomCommand)
	...
	mux.HandleFunc("/api/v1/tunnel/update-job/get", h.tunnelUpdateJobGet)
	...
	mux.HandleFunc("/api/v1/cover/profile/list", h.coverProfileList)
	mux.HandleFunc("/api/v1/cover/profile/get", h.coverProfileGet)
	mux.HandleFunc("/api/v1/cover/profile/update", h.coverProfileUpdate)
	mux.HandleFunc("/api/v1/cover/profile/delete", h.coverProfileDelete)
	mux.HandleFunc("/api/v1/cover/tunnel/list", h.coverTunnelList)
	mux.HandleFunc("/api/v1/cover/tunnel/get", h.coverTunnelGet)
	mux.HandleFunc("/api/v1/cover/tunnel/update", h.coverTunnelUpdate)
	mux.HandleFunc("/api/v1/cover/node/list", h.coverNodeList)
	mux.HandleFunc("/api/v1/cover/node/get", h.coverNodeGet)
	mux.HandleFunc("/api/v1/cover/node/update", h.coverNodeUpdate)
	mux.HandleFunc("/api/v1/cover/node/sync", h.coverNodeSync)
	...
	mux.HandleFunc("/api/v1/forward/entry-status", h.forwardEntryStatus)
	mux.HandleFunc("/api/v1/forward/entry-status/batch", h.forwardEntryStatusBatch)
	mux.HandleFunc("/api/v1/forward/target-status", h.forwardTargetStatus)
	mux.HandleFunc("/api/v1/forward/target-status/batch", h.forwardTargetStatusBatch)
	mux.HandleFunc("/api/v1/forward/sla-status/batch", h.forwardSLAStatusBatch)
	...
	mux.HandleFunc("/api/v1/forward/batch-update-strategy", h.forwardBatchUpdateStrategy)
	mux.HandleFunc("/api/v1/forward/batch-job/start", h.forwardBatchJobStart)
	mux.HandleFunc("/api/v1/forward/batch-job/get", h.forwardBatchJobGet)
