...
const forwardBatchWorkerLimit = 8

const (
	forwardBatchActionDelete         = "delete"
	forwardBatchActionPause          = "pause"
	forwardBatchActionResume         = "resume"
	forwardBatchActionRedeploy       = "redeploy"
	forwardBatchActionChangeTunnel   = "change_tunnel"
	forwardBatchActionUpdateStrategy = "update_strategy"
)
...
func (h *Handler) nodeUpdate(w http.ResponseWriter, r *http.Request) {
	...
	tunnelIDs, forwardIDs, err := h.collectNodeDependentRuntimeIDs(id)
	if err != nil {
		fmt.Printf("node update runtime reconcile: node %d dependency lookup failed: %v\n", id, err)
	} else {
		h.redeployNodeDependentTunnels(tunnelIDs)
		h.refreshNodeDependentForwardState(forwardIDs)
	}
	response.WriteJSON(w, response.OKEmpty())
}
...
func (h *Handler) nodeInstall(w http.ResponseWriter, r *http.Request) {
	...
	if channel == releaseChannelPatched {
		scriptURL := h.buildPanelAgentAssetURL(r, "install.sh")
		downloadTemplate, _, ok := h.buildPanelPatchedAgentURLs(r)
		if scriptURL == "" || !ok {
			response.WriteJSON(w, response.Err(-2, "当前面板未提供内建版节点安装包"))
			return
		}
		cmd = fmt.Sprintf("curl -L %s -o ./install.sh && chmod +x ./install.sh && PROXY_ENABLED=false DOWNLOAD_URL_TEMPLATE=%s ./install.sh -a %s -s %s",
			scriptURL, downloadTemplate, processServerAddress(panelAddr), secret)
	} else {
...
func (h *Handler) tunnelCreate(w http.ResponseWriter, r *http.Request) {
	...
	tunnelProtocol, protocolErr := resolveTunnelPrimaryProtocol(req, typeVal)
	if protocolErr != nil {
		response.WriteJSON(w, response.ErrDefault(protocolErr.Error()))
		return
	}
	now := time.Now().UnixMilli()
			...
			if typeVal == 1 {
				...
				if len(runtimeState.InNodes) > 0 {
					...
					if len(inNodesRaw) > 0 {
						...
						normalizedTargetProto, normalizeErr := normalizeTunnelProtocolWithDefault(asString(inNodesRaw[0]["protocol"]), "tcp")
						if normalizeErr != nil {
							response.WriteJSON(w, response.ErrDefault(normalizeErr.Error()))
							return
						}
						targetProto = normalizedTargetProto
					}
				}
...
func (h *Handler) tunnelUpdate(w http.ResponseWriter, r *http.Request) {
	...
	actorUserID, actorRole, actorErr := userRoleFromRequest(r)
	if actorErr != nil {
	...
	if typeVal == 1 {
		job := h.startTunnelForwardResyncJob(id, actorUserID, actorRole, oldEntryNodeIDs, newEntryNodeIDs)
		if job != nil {
			response.WriteJSON(w, response.OK(job))
			return
		}
		response.WriteJSON(w, response.OKEmpty())
		return
...
func (h *Handler) startTunnelForwardResyncJob(tunnelID int64, actorUserID int64, actorRole int, oldEntryNodeIDs, newEntryNodeIDs []int64) *batchJobSnapshot {
	if h == nil || h.batchJobs == nil || tunnelID <= 0 {
		return nil
	}
	...
	job := h.batchJobs.create("tunnel_update", actorUserID, actorRole, total)
	if job == nil {
		return nil
	}

	forwardCopies := make([]forwardRecord, len(forwards))
	copy(forwardCopies, forwards)
	oldIDs := append([]int64(nil), oldEntryNodeIDs...)
	newIDs := append([]int64(nil), newEntryNodeIDs...)

	go func(jobID string) {
		h.batchJobs.markRunning(jobID)

		if !sameInt64Set(oldIDs, newIDs) {
			h.cleanupTunnelForwardRuntimesOnRemovedEntryNodes(tunnelID, oldIDs, newIDs)
			h.syncTunnelForwardsEntryPorts(tunnelID, newIDs)
			h.batchJobs.applyResult(jobID, forwardBatchTaskResult{
				ID:      tunnelID,
		...
		if len(forwardCopies) > 0 {
			runForwardBatchTasksWithEmitter(uniqueForwardIDs(forwardCopies), func(id int64) forwardBatchTaskResult {
				taskResult := forwardBatchTaskResult{ID: id}
				var forward *forwardRecord
				for i := range forwardCopies {
					if forwardCopies[i].ID == id {
						forward = &forwardCopies[i]
						break
					}
				}
				if forward == nil {
					taskResult.Err = errors.New("转发不存在")
					return taskResult
				}
				taskResult.Name = forward.Name
				...
				return taskResult
			}, func(result forwardBatchTaskResult) {
				h.batchJobs.applyResult(jobID, result)
			})
		}

		h.batchJobs.complete(jobID)
	}(job.JobID)

	return job
}

func uniqueForwardIDs(forwards []forwardRecord) []int64 {
	...
	for _, forward := range forwards {
		...
		if _, ok := seen[forward.ID]; ok {
			continue
		}
		seen[forward.ID] = struct{}{}
...
func (h *Handler) validateForwardPortAvailabilityTx(tx *gorm.DB, node *nodeRecord, port int, currentForwardID int64, mode, bindIP string) error {
	if h == nil || h.repo == nil || tx == nil || node == nil || port <= 0 {
		return nil
	}
	occupants, err := h.repo.ListOtherForwardPortOccupantsTx(tx, node.ID, port, currentForwardID)
	if err != nil {
		return err
	}
	return validateForwardPortOccupants(node.Name, port, mode, bindIP, occupants)
}
...
func (h *Handler) validateTunnelEntryPortConflictsForNewEntriesTx(tx *gorm.DB, tunnelID int64, oldEntryNodeIDs, newEntryNodeIDs []int64) error {
	...
	for i := range forwards {
		...
		for _, nodeID := range addedNodeIDs {
			...
			bindIP := ""
			if len(newEntryNodeIDs) == 1 {
				bindIP = firstForwardBindIP(oldPorts)
			}
			if err := h.validateForwardPortAvailabilityTx(tx, node, port, f.ID, f.Mode, bindIP); err != nil {
...
func (h *Handler) syncTunnelForwardsEntryPorts(tunnelID int64, entryNodeIDs []int64) {
	...
	for i := range forwards {
		...
		preservedInIP := ""
		if allowInIP {
			preservedInIP = firstForwardBindIP(oldPorts)
		}
		...
		for _, nid := range entryNodeIDs {
			...
			port := h.resolvePortForNewEntryNode(nid, referencePort, f.ID, f.Mode, preservedInIP)
			inIP := preservedInIP
...
func (h *Handler) resolvePortForNewEntryNode(nodeID int64, referencePort int, forwardID int64, mode, bindIP string) int {
	...
	if validateLocalNodePort(node, referencePort) == nil &&
		...
		if err := h.validateForwardPortAvailability(node, referencePort, forwardID, mode, bindIP); err == nil {
			return referencePort
		}
	}
...
type tunnelUpdateJobRequest struct {
	JobID string `json:"jobId"`
}

type forwardBatchTaskResult struct {
	ID      int64
	Name    string
	Success bool
	Err     error
...
func (h *Handler) tunnelUpdateJobGet(w http.ResponseWriter, r *http.Request) {
	var req tunnelUpdateJobRequest
	if err := decodeJSON(r.Body, &req); err != nil || strings.TrimSpace(req.JobID) == "" {
		response.WriteJSON(w, response.ErrDefault("请求参数错误"))
		return
	}
	...
	if h.batchJobs == nil {
		response.WriteJSON(w, response.Err(-2, "batch job manager unavailable"))
		return
	}

	job, ok := h.batchJobs.get(req.JobID)
	if !ok || job == nil {
		response.WriteJSON(w, response.ErrDefault("任务不存在"))
		return
	}
	if actorRole != 0 && job.OwnerUserID != actorUserID {
		response.WriteJSON(w, response.ErrDefault("任务不存在"))
		return
	}
...
func clampForwardBatchWorkers(total int) int {
	if total <= 0 {
		return 1
	}
	if total < forwardBatchWorkerLimit {
		return total
	}
	return forwardBatchWorkerLimit
}

func runForwardBatchTasks(ids []int64, fn func(id int64) forwardBatchTaskResult) batchOperationResult {
	return runForwardBatchTasksWithEmitter(ids, fn, nil)
}

func runForwardBatchTasksWithEmitter(ids []int64, fn func(id int64) forwardBatchTaskResult, onResult func(forwardBatchTaskResult)) batchOperationResult {
	if len(ids) == 0 {
	...
	type indexedResult struct {
		Index  int
		Result forwardBatchTaskResult
	}

	jobs := make(chan indexedJob, len(ids))
	results := make(chan indexedResult, len(ids))
	workerCount := clampForwardBatchWorkers(len(ids))
	...
	for idx, id := range ids {
		jobs <- indexedJob{Index: idx, ID: id}
	}
	close(jobs)
	...
	indexedFailures := make([]struct {
		Index   int
		Failure batchFailureDetail
	}, 0)
	outcome := batchOperationResult{}
	...
	for item := range results {
		...
		outcome.FailCount++
		reason := normalizeBatchFailureReason(errString(item.Result.Err))
		if reason == "" {
			reason = "未知错误"
		}
		indexedFailures = append(indexedFailures, struct {
			Index   int
			Failure batchFailureDetail
		}{
			Index: item.Index,
	...
	if len(indexedFailures) > 0 {
		sort.Slice(indexedFailures, func(i, j int) bool {
			return indexedFailures[i].Index < indexedFailures[j].Index
		})
		outcome.Failures = make([]batchFailureDetail, 0, len(indexedFailures))
		for _, item := range indexedFailures {
			outcome.Failures = append(outcome.Failures, item.Failure)
		}
	}
...
func (h *Handler) forwardCreate(w http.ResponseWriter, r *http.Request) {
	...
	name := asString(req["name"])
	mode, err := normalizeForwardMode(asString(req["mode"]))
	if err != nil {
		response.WriteJSON(w, response.ErrDefault(err.Error()))
		return
	}
	sniRules := strings.TrimSpace(asString(req["sniRules"]))
	remoteAddr, err := deriveForwardRemoteAddr(mode, asString(req["remoteAddr"]), sniRules)
	if err != nil {
		response.WriteJSON(w, response.ErrDefault(err.Error()))
		return
	}
	...
	for _, nodeID := range entryNodes {
		...
		if err := h.validateForwardPortAvailability(node, port, 0, mode, inIp); err != nil {
			response.WriteJSON(w, response.ErrDefault(err.Error()))
			return
		}
	}
	...
	forwardID, err := h.repo.CreateForwardTx(userID, userName, name, tunnelID, mode, remoteAddr, sniRules, defaultString(asString(req["strategy"]), "fifo"), now, inx, entryNodes, port, inIp, nullableInt(speedID))
	if err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	...
	h.invalidateForwardEntrySummary(createdForward.ID)
	h.enqueueForwardEntrySummaryRefresh(createdForward.ID)
	response.WriteJSON(w, response.OKEmpty())
}
...
func (h *Handler) forwardUpdate(w http.ResponseWriter, r *http.Request) {
	...
	mode := normalizeStoredForwardMode(forward.Mode)
	if rawMode, ok := req["mode"]; ok {
		mode, err = normalizeForwardMode(asString(rawMode))
		if err != nil {
			response.WriteJSON(w, response.ErrDefault(err.Error()))
			return
		}
	...
	if rawRules, ok := req["sniRules"]; ok {
		sniRules = strings.TrimSpace(asString(rawRules))
	}
	remoteAddrInput := forward.RemoteAddr
	if rawRemoteAddr, ok := req["remoteAddr"]; ok {
		remoteAddrInput = asString(rawRemoteAddr)
	}
	remoteAddr, err := deriveForwardRemoteAddr(mode, remoteAddrInput, sniRules)
	if err != nil {
		response.WriteJSON(w, response.ErrDefault(err.Error()))
		return
	}
	...
	effectiveInIP := resolvedForwardBindIP(hasInIP, inIp, oldPorts, len(fwdEntryNodes), tunnelChanged)
	for _, nodeID := range fwdEntryNodes {
		...
		if err := h.validateForwardPortAvailability(node, port, id, mode, effectiveInIP); err != nil {
			response.WriteJSON(w, response.ErrDefault(err.Error()))
			return
		}
	}
	now := time.Now().UnixMilli()
	if err := h.repo.UpdateForward(id, name, tunnelID, mode, remoteAddr, sniRules, strategy, now, newSpeedID); err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	if hasInIP {
	...
	if len(warnings) > 0 {
		h.invalidateForwardEntrySummary(updatedForward.ID)
		h.enqueueForwardEntrySummaryRefresh(updatedForward.ID)
...
func (h *Handler) forwardDelete(w http.ResponseWriter, r *http.Request) {
	...
	h.invalidateForwardEntrySummary(id)
	response.WriteJSON(w, response.OKEmpty())
}
...
func (h *Handler) forwardBatchDelete(w http.ResponseWriter, r *http.Request) {
	...
	result := runForwardBatchTasks(ids, func(id int64) forwardBatchTaskResult {
		taskResult := forwardBatchTaskResult{ID: id}
		forward, accessErr := h.ensureForwardAccessByActor(actorUserID, actorRole, id)
		if accessErr != nil {
			taskResult.Err = accessErr
			return taskResult
		}
		taskResult.Name = forward.Name
		if err := h.controlForwardServices(forward, "DeleteService", true); err != nil {
...
func (h *Handler) forwardEntryStatus(w http.ResponseWriter, r *http.Request) {
	var req struct {
		ID int64 `json:"id"`
	}
	...
	forward, _, _, err := h.resolveForwardAccess(r, req.ID)
	if err != nil {
		if errors.Is(err, errForwardNotFound) {
			response.WriteJSON(w, response.ErrDefault("转发不存在"))
			return
	...
	items, err := h.inspectForwardEntrypoints(forward)
	if err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
...
func (h *Handler) forwardEntryStatusBatch(w http.ResponseWriter, r *http.Request) {
	var req struct {
		IDs   []int64 `json:"ids"`
	...
	items := make([]forwardEntryStatusSummary, 0, len(req.IDs))
	for _, id := range req.IDs {
		...
		if req.Force {
			h.invalidateForwardEntrySummary(forward.ID)
			summary, summaryErr := h.computeForwardEntrySummary(forward)
			if summaryErr != nil {
				items = append(items, buildUnknownForwardEntrySummary(forward))
				continue
			}
			h.storeForwardEntrySummary(forward.ID, summary)
			items = append(items, summary)
			continue
		}
		if cached, fresh := h.getCachedForwardEntrySummary(forward.ID); fresh {
			items = append(items, cached)
			continue
		} else if cached.ForwardID > 0 {
			items = append(items, cached)
		} else {
			items = append(items, buildUnknownForwardEntrySummary(forward))
		}
		h.enqueueForwardEntrySummaryRefresh(forward.ID)
	}

	response.WriteJSON(w, response.OK(items))
}

func (h *Handler) forwardTargetStatus(w http.ResponseWriter, r *http.Request) {
	var req struct {
		ID int64 `json:"id"`
	}
	if err := decodeJSON(r.Body, &req); err != nil || req.ID <= 0 {
	...
	response.WriteJSON(w, response.OK(summarizeForwardTargets(forward, results)))
}

func (h *Handler) forwardTargetStatusBatch(w http.ResponseWriter, r *http.Request) {
	var req struct {
		IDs   []int64 `json:"ids"`
		Force bool    `json:"force"`
	}
	...
	items := make([]forwardTargetStatusSummary, 0, len(req.IDs))
	for _, id := range req.IDs {
		if id <= 0 {
			continue
		}
		...
		if req.Force {
			h.invalidateForwardEntrySummary(forward.ID)
			summary, summaryErr := h.computeForwardTargetSummary(forward)
			if summaryErr != nil {
				items = append(items, buildUnknownForwardTargetSummary(forward))
				continue
			}
			h.storeForwardTargetSummary(forward.ID, summary)
			items = append(items, summary)
			continue
		}
		if cached, fresh := h.getCachedForwardTargetSummary(forward.ID); fresh {
			items = append(items, cached)
			continue
		} else if cached.ForwardID > 0 {
			items = append(items, cached)
		...
		h.enqueueForwardTargetSummaryRefresh(forward.ID)
	}
...
func normalizeForwardStrategyValue(value string) (string, error) {
	strategy := strings.ToLower(strings.TrimSpace(value))
	switch strategy {
	case "fifo", "round", "rand", "hash":
		return strategy, nil
	default:
		return "", fmt.Errorf("无效的负载均衡策略")
	}
}

func (h *Handler) applyForwardStrategyChange(forward *forwardRecord, strategy string) error {
	if h == nil || forward == nil {
		return errors.New("invalid forward strategy context")
	}

	normalizedStrategy, err := normalizeForwardStrategyValue(strategy)
	if err != nil {
		return err
	}
	if strings.EqualFold(strings.TrimSpace(forward.Strategy), normalizedStrategy) {
		return nil
	}
	...
	desiredForward.Strategy = normalizedStrategy

	// Active rules should first go through PauseService so the agent can run its
	// stronger port cleanup path before we delete and recreate listeners.
	if forward.Status == 1 {
		_ = h.controlForwardServices(forward, "PauseService", true)
	}
	...
	for _, wait := range waits {
		...
		applyErr = h.syncForwardServices(&desiredForward, "AddService", false)
		if applyErr == nil {
			break
		}
		if !isAddressAlreadyInUseError(applyErr) {
			break
		}
	...
	if applyErr != nil {
		if isAddressAlreadyInUseError(applyErr) {
			oldPort := pickForwardPortFromRecords(oldPorts)
			newPort, _, pickErr := h.pickTunnelPortRuntimeAware(forward.TunnelID)
			if pickErr == nil && newPort > 0 && newPort != oldPort {
				if replaceErr := h.replaceForwardPortsPreservingInIP(forward.ID, forward.TunnelID, newPort, oldPorts); replaceErr == nil {
					_ = h.controlForwardServices(&desiredForward, "DeleteService", true)
					...
					if applyErr == nil {
						goto persistForwardStrategy
					}
				}
				_ = h.replaceForwardPortsWithRecords(forward.ID, oldPorts)
			}
		}
		if isAddressAlreadyInUseError(applyErr) {
			if ports, portsErr := h.listForwardPorts(forward.ID); portsErr == nil {
				for _, fp := range ports {
					...
					cmdline := strings.TrimSpace(asString(info["cmdline"]))
					exe := strings.TrimSpace(asString(info["exe"]))
					if cmdline != "" || exe != "" {
						details := cmdline
						if details == "" {
							details = exe
						}
						_ = h.syncForwardServices(forward, "AddService", false)
						return fmt.Errorf("%w；节点 %d 端口 %d 已被外部进程占用：%s", applyErr, fp.NodeID, fp.Port, details)
					}
				}
			}
		}
...
persistForwardStrategy:
	if err := h.repo.UpdateForward(
		forward.ID,
		forward.Name,
		forward.TunnelID,
		forward.Mode,
...
func (h *Handler) applyForwardTunnelChange(forward *forwardRecord, actorUserID int64, actorRole int, targetTunnelID int64) error {
	if h == nil || forward == nil {
		return errors.New("invalid forward tunnel change context")
	}
	if targetTunnelID <= 0 {
		return errors.New("目标隧道不存在")
	}
	if forward.TunnelID == targetTunnelID {
		return errors.New("规则已在目标隧道中")
	}

	if err := h.ensureTunnelPermission(actorUserID, actorRole, targetTunnelID); err != nil {
		return err
	}
	...
	if targetTunnel.Status != 1 {
		return errors.New("目标隧道已禁用")
	}

	oldPorts, listPortsErr := h.listForwardPorts(forward.ID)
	if listPortsErr != nil {
		return listPortsErr
	}
	if len(oldPorts) == 0 {
	...
	oldNodeIDs := forwardPortNodeIDs(oldPorts)
	port := h.repo.GetMinForwardPort(forward.ID)
	if err := h.repo.UpdateForwardTunnel(forward.ID, targetTunnelID, time.Now().UnixMilli()); err != nil {
		return err
	}
	...
	if p <= 0 {
		p = h.pickTunnelPort(targetTunnelID)
	}

	bctEntryNodes, _ := h.tunnelEntryNodeIDs(targetTunnelID)
	...
	for _, nid := range bctEntryNodes {
		...
		if validateErr := h.validateForwardPortAvailability(nd, p, forward.ID, forward.Mode, ""); validateErr != nil {
			h.rollbackForwardMutation(forward, oldPorts)
			return validateErr
		}
	}

	if err := h.replaceForwardPorts(forward.ID, targetTunnelID, p, ""); err != nil {
		h.rollbackForwardMutation(forward, oldPorts)
		return err
	}

	updatedForward, fetchErr := h.getForwardRecord(forward.ID)
	if fetchErr != nil {
		h.rollbackForwardMutation(forward, oldPorts)
		return fetchErr
	}
...
func (h *Handler) buildForwardBatchTask(action string, actorUserID int64, actorRole int, strategy string, targetTunnelID int64) (func(id int64) forwardBatchTaskResult, error) {
	normalizedAction := strings.ToLower(strings.TrimSpace(action))
	switch normalizedAction {
	case forwardBatchActionDelete:
		return func(id int64) forwardBatchTaskResult {
			taskResult := forwardBatchTaskResult{ID: id}
			forward, accessErr := h.ensureForwardAccessByActor(actorUserID, actorRole, id)
			if accessErr != nil {
				taskResult.Err = accessErr
	...
	case forwardBatchActionRedeploy:
		return func(id int64) forwardBatchTaskResult {
			taskResult := forwardBatchTaskResult{ID: id}
			forward, accessErr := h.ensureForwardAccessByActor(actorUserID, actorRole, id)
			if accessErr != nil {
	...
	case forwardBatchActionChangeTunnel:
		if targetTunnelID <= 0 {
			return nil, errors.New("目标隧道不存在")
		}
		return func(id int64) forwardBatchTaskResult {
			taskResult := forwardBatchTaskResult{ID: id}
			forward, accessErr := h.ensureForwardAccessByActor(actorUserID, actorRole, id)
			...
			taskResult.Name = forward.Name
			if err := h.applyForwardTunnelChange(forward, actorUserID, actorRole, targetTunnelID); err != nil {
				taskResult.Err = err
				return taskResult
			}
			taskResult.Success = true
	...
	case forwardBatchActionUpdateStrategy:
		normalizedStrategy, err := normalizeForwardStrategyValue(strategy)
		if err != nil {
			return nil, err
		}
		return func(id int64) forwardBatchTaskResult {
			...
			taskResult.Name = forward.Name
			if err := h.applyForwardStrategyChange(forward, normalizedStrategy); err != nil {
				taskResult.Err = err
				return taskResult
			}
			taskResult.Success = true
	...
	default:
		return nil, fmt.Errorf("unsupported batch action: %s", normalizedAction)
	}
}

func (h *Handler) forwardBatchUpdateStrategy(w http.ResponseWriter, r *http.Request) {
	var req struct {
		IDs      []int64 `json:"ids"`
	...
	strategy, err := normalizeForwardStrategyValue(req.Strategy)
	if err != nil {
		response.WriteJSON(w, response.ErrDefault(err.Error()))
		return
	}
	...
	result := runForwardBatchTasks(req.IDs, func(id int64) forwardBatchTaskResult {
		taskResult := forwardBatchTaskResult{ID: id}
		...
		if strings.EqualFold(strings.TrimSpace(forward.Strategy), strategy) {
			taskResult.Success = true
			return taskResult
		}

		if err := h.applyForwardStrategyChange(forward, strategy); err != nil {
			taskResult.Err = err
			return taskResult
		}
...
func (h *Handler) forwardBatchJobStart(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	taskFn, err := h.buildForwardBatchTask(req.Action, actorUserID, actorRole, req.Strategy, req.TargetTunnelID)
	if err != nil {
		response.WriteJSON(w, response.ErrDefault(err.Error()))
		return
	}
	...
	job := h.batchJobs.create(req.Action, actorUserID, actorRole, len(req.IDs))
	if job == nil {
		response.WriteJSON(w, response.Err(-2, "failed to create batch job"))
		return
	}

	go func(jobID string, ids []int64) {
		h.batchJobs.markRunning(jobID)
		runForwardBatchTasksWithEmitter(ids, taskFn, func(result forwardBatchTaskResult) {
			h.batchJobs.applyResult(jobID, result)
		})
		h.batchJobs.complete(jobID)
	}(job.JobID, append([]int64(nil), req.IDs...))

	response.WriteJSON(w, response.OK(job))
}

func (h *Handler) forwardBatchJobGet(w http.ResponseWriter, r *http.Request) {
	var req struct {
		JobID string `json:"jobId"`
	}
	if err := decodeJSON(r.Body, &req); err != nil || strings.TrimSpace(req.JobID) == "" {
...
func (h *Handler) forwardBatchChangeTunnel(w http.ResponseWriter, r *http.Request) {
	...
	for _, id := range req.ForwardIDs {
		...
		for _, nid := range bctEntryNodes {
			...
			if validateErr := h.validateForwardPortAvailability(nd, p, id, forward.Mode, ""); validateErr != nil {
				portRangeOk = false
				portRangeErr = validateErr
				break
			}
...
func (h *Handler) prepareTunnelCreateState(tx *gorm.DB, req map[string]interface{}, tunnelType int, excludeTunnelID int64) (*tunnelCreateState, error) {
	...
	for _, item := range asMapSlice(req["inNodeId"]) {
		...
		protocol, protocolErr := normalizeTunnelProtocol(asString(item["protocol"]))
		if protocolErr != nil {
...
func retryTunnelServiceAddWithCleanup(add func() error, cleanup func() error, wait time.Duration) error {
	return retryServiceAddWithCleanupOnBindConflict(add, cleanup, wait)
}
...
var allowedTunnelProtocols = map[string]struct{}{
	"tls":  {},
	"wss":  {},
	"tcp":  {},
	"mtls": {},
...
func normalizeTunnelProtocol(raw string) (string, error) {
	return normalizeTunnelProtocolWithDefault(raw, "tls")
}

func normalizeTunnelProtocolWithDefault(raw, fallback string) (string, error) {
	value := strings.TrimSpace(strings.ToLower(raw))
	if value == "" {
		value = strings.TrimSpace(strings.ToLower(defaultString(fallback, "tls")))
	}
	if _, ok := allowedTunnelProtocols[value]; !ok {
		return "", fmt.Errorf("不支持的隧道协议: %s", strings.TrimSpace(raw))
	}
	return value, nil
}

func resolveTunnelPrimaryProtocol(req map[string]interface{}, tunnelType int) (string, error) {
	if tunnelType == 2 {
		for _, item := range asMapSlice(req["outNodeId"]) {
			if protocol, err := normalizeTunnelProtocol(asString(item["protocol"])); err == nil {
				return protocol, nil
			} else if strings.TrimSpace(asString(item["protocol"])) != "" {
				return "", err
			}
		}
		for _, group := range asAnySlice(req["chainNodes"]) {
			for _, item := range asMapSlice(group) {
				if protocol, err := normalizeTunnelProtocol(asString(item["protocol"])); err == nil {
					return protocol, nil
				} else if strings.TrimSpace(asString(item["protocol"])) != "" {
					return "", err
...
func isTLSTunnelProtocol(protocol string) bool {
	switch strings.TrimSpace(strings.ToLower(defaultString(protocol, "tls"))) {
...
func (h *Handler) replaceTunnelChainsTx(tx *gorm.DB, tunnelID int64, req map[string]interface{}) error {
	...
	for i, n := range inNodes {
		...
		protocol, protocolErr := normalizeTunnelProtocol(asString(n["protocol"]))
		if protocolErr != nil {
			return protocolErr
		}
...
func (h *Handler) deleteNodeByID(id int64) error {
	...
	cleanupTunnelIDs, err := h.repo.ListTunnelIDsByNode(id)
	if err != nil {
		fmt.Printf("node delete runtime reconcile: node %d tunnel lookup failed: %v\n", id, err)
	}
	redeployTunnelIDs, err := h.repo.ListActiveTunnelIDsByNode(id)
	if err != nil {
		fmt.Printf("node delete runtime reconcile: node %d active tunnel lookup failed: %v\n", id, err)
	}
	forwardIDs, err := h.repo.ListForwardIDsByNode(id)
	if err != nil {
		fmt.Printf("node delete runtime reconcile: node %d forward lookup failed: %v\n", id, err)
	}

	h.cleanupNodeDependentRuntimeBeforeDelete(id, cleanupTunnelIDs, forwardIDs)

	if err := h.repo.DeleteNodeCascade(id); err != nil {
		return err
	}

	h.redeployNodeDependentTunnels(redeployTunnelIDs)
	h.refreshNodeDependentForwardState(forwardIDs)
	return nil
}
...
func (h *Handler) collectNodeDependentRuntimeIDs(nodeID int64) ([]int64, []int64, error) {
	...
	return uniqueInt64s(tunnelIDs), uniqueInt64s(forwardIDs), nil
}

func (h *Handler) cleanupNodeDependentRuntimeBeforeDelete(nodeID int64, tunnelIDs, forwardIDs []int64) {
	if h == nil {
		return
	}
	...
	for _, forwardID := range uniqueInt64s(forwardIDs) {
		if forwardID <= 0 {
			continue
		}
		...
		if err := h.deleteForwardServicesOnNodeBatch(forward, nodeID); err != nil {
			fmt.Printf("node delete cleanup: forward %d on node %d failed: %v\n", forwardID, nodeID, err)
		}
	}
}

func (h *Handler) redeployNodeDependentTunnels(tunnelIDs []int64) {
	if h == nil {
		return
	}
	...
	for _, tunnelID := range uniqueInt64s(tunnelIDs) {
		...
		if err := h.redeployTunnelAndForwards(tunnelID); err != nil {
			fmt.Printf("node runtime reconcile: tunnel %d failed: %v\n", tunnelID, err)
		}
	}
}

func (h *Handler) refreshNodeDependentForwardState(forwardIDs []int64) {
	if h == nil || h.repo == nil {
		return
	}

	forwardIDs = uniqueInt64s(forwardIDs)
	if len(forwardIDs) == 0 {
		return
	}

	_ = h.repo.DeleteForwardSLAStates(forwardIDs)
	for _, forwardID := range forwardIDs {
		if forwardID <= 0 {
			continue
		}
		h.invalidateForwardEntrySummary(forwardID)
		h.enqueueForwardEntrySummaryRefresh(forwardID)
		h.enqueueForwardTargetSummaryRefresh(forwardID)
	}
}
...
func (h *Handler) pickTunnelPortRuntimeAware(tunnelID int64) (int, string, error) {
	entryNodes, err := h.tunnelEntryNodeIDs(tunnelID)
	if err != nil || len(entryNodes) == 0 {
	...
	lastOccupiedReason := ""
	for _, candidate := range commonAvailable {
		occupied := false
		for _, nodeID := range entryNodes {
			inUse, reason, err := h.isNodePortExternallyOccupied(nodeID, candidate)
			if err != nil {
				continue
			}
			if inUse {
				occupied = true
				lastOccupiedReason = reason
				break
			}
		}
		if !occupied {
	...
	if lastOccupiedReason == "" {
		lastOccupiedReason = "所有候选端口都被外部进程占用"
	}
	return 0, lastOccupiedReason, errors.New(lastOccupiedReason)
}

func (h *Handler) isNodePortExternallyOccupied(nodeID int64, port int) (bool, string, error) {
	...
	info, err := h.inspectNodePort(nodeID, port)
	if err != nil || info == nil {
		return false, "", err
	}
	...
	if !strings.Contains(ss, "LISTEN") && lsof == "" {
		return false, "", nil
	}

	cmdline := strings.TrimSpace(asString(info["cmdline"]))
	exe := strings.TrimSpace(asString(info["exe"]))
	ps := strings.TrimSpace(asString(info["ps"]))
	reason := strings.TrimSpace(cmdline)
	if reason == "" {
		reason = strings.TrimSpace(exe)
	}
	if reason == "" {
		reason = strings.TrimSpace(ps)
	}
	if reason == "" {
		reason = fmt.Sprintf("节点 %s 端口 %d 被未知进程占用", node.Name, port)
	}
...
func (h *Handler) rollbackForwardMutation(oldForward *forwardRecord, oldPorts []forwardPortRecord) {
	...
	h.repo.RollbackForwardFields(
		...
		oldForward.TunnelID, oldForward.Mode, oldForward.RemoteAddr, oldForward.SniRules, oldForward.Strategy, oldForward.Status,
		oldForward.SpeedID,
		time.Now().UnixMilli(),
	)
...
func (h *Handler) validateForwardPortAvailability(node *nodeRecord, port int, currentForwardID int64, mode, bindIP string) error {
	if h == nil || h.repo == nil || node == nil || port <= 0 {
		return nil
	}
	occupants, err := h.repo.ListOtherForwardPortOccupants(node.ID, port, currentForwardID)
	if err != nil {
		return err
	}
	return validateForwardPortOccupants(node.Name, port, mode, bindIP, occupants)
...
func validateForwardPortOccupants(nodeName string, port int, mode, bindIP string, occupants []repo.ForwardPortOccupant) error {
	if len(occupants) == 0 {
		return nil
	}

	requestedSNI := isSNIForwardMode(mode)
	normalizedBindIP := strings.TrimSpace(bindIP)
	for _, occupant := range occupants {
		if !requestedSNI || !strings.EqualFold(strings.TrimSpace(occupant.Mode), forwardModeSNI) {
			return fmt.Errorf("节点 %s 端口 %d 已被其他转发占用", nodeName, port)
		}
		if strings.TrimSpace(occupant.InIP) != normalizedBindIP {
			return fmt.Errorf("节点 %s 端口 %d 的 SNI 共享入口IP必须保持一致", nodeName, port)
		}
	}
...
func firstForwardBindIP(ports []forwardPortRecord) string {
	for _, fp := range ports {
		if strings.TrimSpace(fp.InIP) != "" {
			return strings.TrimSpace(fp.InIP)
		}
	}
	return ""
}

func resolvedForwardBindIP(hasExplicit bool, explicit string, oldPorts []forwardPortRecord, entryNodeCount int, tunnelChanged bool) string {
	if hasExplicit {
		return strings.TrimSpace(explicit)
	}
	if tunnelChanged || entryNodeCount != 1 {
		return ""
	}
	return firstForwardBindIP(oldPorts)
}
