...
func (s *Server) handleNode(w http.ResponseWriter, r *http.Request, nodeID int64, secret string) {
	...
	observedIP := extractObservedClientIP(r)
	...
	ns := &nodeSession{nodeID: nodeID, secret: secret, conn: cw, crypto: nodeCrypto, observedIP: observedIP}
	s.nodes[nodeID] = ns
	s.byConn[conn] = ns
	s.mu.Unlock()
...
func (s *Server) GetObservedNodeIP(nodeID int64) string {
	if s == nil || nodeID <= 0 {
		return ""
	}
	s.mu.RLock()
	...
	return strings.TrimSpace(session.observedIP)
}
...
func extractObservedClientIP(r *http.Request) string {
	if r == nil {
		return ""
	}
...
func parseObservedIP(raw string) string {
	for _, part := range strings.Split(strings.TrimSpace(raw), ",") {
		...
		if host, _, err := net.SplitHostPort(candidate); err == nil {
			candidate = strings.Trim(strings.TrimSpace(host), "\"[]")
		}
