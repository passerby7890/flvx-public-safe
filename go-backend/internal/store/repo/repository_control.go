...
func (r *Repository) ListForwardsByTunnelTx(tx *gorm.DB, tunnelID int64) ([]model.ForwardRecord, error) {
	...
	for i := range rows {
		if strings.TrimSpace(rows[i].Mode) == "" {
			rows[i].Mode = "direct"
		}
...
func (r *Repository) ListTunnelIDsByNode(nodeID int64) ([]int64, error) {
	if r == nil || r.db == nil {
		return nil, errors.New("repository not initialized")
	}
	var ids []int64
...
type ForwardPortOccupant struct {
	ForwardID int64
	Mode      string
	InIP      string
}

func (r *Repository) ListOtherForwardPortOccupants(nodeID int64, port int, currentForwardID int64) ([]ForwardPortOccupant, error) {
	if r == nil || r.db == nil {
		return nil, errors.New("repository not initialized")
	}
	return r.ListOtherForwardPortOccupantsTx(r.db, nodeID, port, currentForwardID)
}

func (r *Repository) ListOtherForwardPortOccupantsTx(tx *gorm.DB, nodeID int64, port int, currentForwardID int64) ([]ForwardPortOccupant, error) {
	if tx == nil {
		return nil, errors.New("database unavailable")
	}
	if nodeID <= 0 || port <= 0 {
	...
	var rows []ForwardPortOccupant
	err := tx.Table("forward_port").
		Joins("JOIN forward ON forward.id = forward_port.forward_id").
		Where("forward_port.node_id = ? AND forward_port.port = ? AND forward_port.forward_id <> ?", nodeID, port, currentForwardID).
		Select("forward_port.forward_id AS forward_id, forward.mode AS mode, COALESCE(forward_port.in_ip, '') AS in_ip").
		Order("forward_port.forward_id ASC").
		Find(&rows).Error
	if err != nil {
		return nil, err
...
func (r *Repository) ListOnlineLocalNodeRecords() ([]model.NodeRecord, error) {
	if r == nil || r.db == nil {
		return nil, errors.New("repository not initialized")
	}

	var nodes []model.Node
	if err := r.db.Where("status = ? AND is_remote = 0", 1).Order("id ASC").Find(&nodes).Error; err != nil {
		return nil, err
	}

	items := make([]model.NodeRecord, 0, len(nodes))
	for i := range nodes {
		if rec := nodeRecordFromModel(&nodes[i]); rec != nil {
...
func (r *Repository) UpdateNodeNetworkInfo(nodeID int64, serverIP, serverIPv4, serverIPv6, extraIPs string, now int64) error {
	if r == nil || r.db == nil {
		return errors.New("repository not initialized")
	}
	if nodeID <= 0 {
	...
	return r.db.Model(&model.Node{}).
		Where("id = ? AND is_remote = 0", nodeID).
		Updates(map[string]interface{}{
			"server_ip":    serverIP,
			"server_ip_v4": sql.NullString{String: strings.TrimSpace(serverIPv4), Valid: strings.TrimSpace(serverIPv4) != ""},
			"server_ip_v6": sql.NullString{String: strings.TrimSpace(serverIPv6), Valid: strings.TrimSpace(serverIPv6) != ""},
			"extra_ips":    sql.NullString{String: strings.TrimSpace(extraIPs), Valid: strings.TrimSpace(extraIPs) != ""},
...
func (r *Repository) ListSNIForwardsOnNode(nodeID int64) ([]model.ForwardRecord, error) {
	if r == nil || r.db == nil {
		return nil, errors.New("repository not initialized")
	}
	var forwards []model.Forward
	err := r.db.Model(&model.Forward{}).
		...
		Where("forward_port.node_id = ? AND forward.status = 1 AND forward.mode = 'sni'", nodeID).
		Select("forward.*").
