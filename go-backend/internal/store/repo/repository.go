...
type TunnelCoverProfile = model.TunnelCoverProfile
type CoverDomainProfile = model.CoverDomainProfile
type TunnelCoverBinding = model.TunnelCoverBinding
type NodeCoverService = model.NodeCoverService
...
func autoMigrateAll(db *gorm.DB) error {
	models := []interface{}{
		...
		&model.TunnelCoverProfile{},
		&model.CoverDomainProfile{},
		&model.TunnelCoverBinding{},
		&model.NodeCoverService{},
		&model.Announcement{},
		&model.SchemaVersion{},
		&model.NodeMetric{},
		&model.TunnelMetric{},
		...
		&model.TunnelQuality{},
		&model.ForwardSLAState{},
		&model.ForwardSLASnapshot{},
	}
...
func (r *Repository) GetUserPackageForwards(userID int64) ([]model.UserForwardDetail, error) {
	...
	err := r.db.Model(&model.Forward{}).
		Select("forward.id, forward.name, forward.tunnel_id, COALESCE(tunnel.name, '') AS tunnel_name, COALESCE(forward.mode, 'direct') AS mode, forward.remote_addr, COALESCE(forward.sni_rules, '') AS sni_rules, forward.in_flow, forward.out_flow, forward.status, forward.created_time AS created_at").
	...
	for _, row := range rows {
		...
		items = append(items, model.UserForwardDetail{
			...
			Mode: row.Mode, RemoteAddr: row.RemoteAddr, SniRules: row.SniRules, InFlow: row.InFlow, OutFlow: row.OutFlow,
...
func (r *Repository) ListForwards() ([]map[string]interface{}, error) {
	...
	var rows []fwdRow
	err := r.db.Model(&model.Forward{}).
		Select("forward.id, forward.user_id, forward.user_name, forward.name, forward.tunnel_id, COALESCE(tunnel.name, '') AS tunnel_name, COALESCE(tunnel.traffic_ratio, 1.0) AS traffic_ratio, COALESCE(forward.mode, 'direct') AS mode, forward.remote_addr, COALESCE(forward.sni_rules, '') AS sni_rules, COALESCE(forward.strategy, 'fifo') AS strategy, forward.in_flow, forward.out_flow, forward.created_time, forward.status, forward.inx, forward.speed_id").
		Joins("LEFT JOIN tunnel ON tunnel.id = forward.tunnel_id").
	...
	for _, row := range rows {
		...
		item := map[string]interface{}{
			...
			"mode": row.Mode, "remoteAddr": row.RemoteAddr, "sniRules": row.SniRules, "strategy": row.Strategy,
...
func (r *Repository) exportForwards() ([]model.ForwardBackup, error) {
	...
	for _, f := range forwards {
		b := model.ForwardBackup{
			...
			TunnelID: f.TunnelID, Mode: f.Mode, RemoteAddr: f.RemoteAddr, SniRules: f.SniRules, Strategy: f.Strategy,
...
func importForwards(tx *gorm.DB, forwards []model.ForwardBackup, now int64) (int, error) {
	...
	for _, f := range forwards {
		...
		err := tx.Clauses(clause.OnConflict{
			Columns: []clause.Column{{Name: "id"}},
			DoUpdates: clause.AssignmentColumns([]string{
				"user_id", "user_name", "name", "tunnel_id", "mode", "remote_addr", "sni_rules", "strategy",
...
func migrateSchema(db *gorm.DB) error {
	...
	normalizeStringColumn := func(modelRef interface{}, table, column, defaultValue string) error {
		result := db.Model(modelRef).Where(fmt.Sprintf("%s IS NULL", column)).Update(column, defaultValue)
		if result.Error != nil {
			msg := strings.ToLower(result.Error.Error())
			if strings.Contains(msg, "no such table") || (strings.Contains(msg, "relation") && strings.Contains(msg, "does not exist")) {
				return nil
			}
			return fmt.Errorf("normalize %s.%s: %w", table, column, result.Error)
		}
		return nil
	}

	if err := normalizeStringColumn(&model.Forward{}, "forward", "mode", "direct"); err != nil {
		return err
	}
	if err := normalizeStringColumn(&model.Forward{}, "forward", "sni_rules", ""); err != nil {
		return err
	}
