...
func (r *Repository) DeleteNodeCascade(nodeID int64) error {
	...
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("node_id = ?", nodeID).Delete(&model.NodeCoverService{}).Error; err != nil {
			return err
		}
...
func (r *Repository) UpdateTunnelTx(tx *gorm.DB, tunnelID int64, name string, typeVal int, flow int64, trafficRatio float64, status int, inIP, protocol, ipPreference string, now int64) error {
	if tx == nil {
...
func (r *Repository) DeleteTunnelCascade(tunnelID int64) error {
	...
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("tunnel_id = ?", tunnelID).Delete(&model.TunnelCoverProfile{}).Error; err != nil {
			return err
		}
...
func (r *Repository) UpdateForward(id int64, name string, tunnelID int64, mode, remoteAddr, sniRules, strategy string, now int64, speedID interface{}) error {
	if r == nil || r.db == nil {
		return errors.New("repository not initialized")
	}
...
func (r *Repository) RollbackForwardFields(id, userID int64, userName, name string, tunnelID int64, mode, remoteAddr, sniRules, strategy string, status int, speedID interface{}, now int64) {
	if r == nil || r.db == nil {
		return
	}
	_ = r.db.Model(&model.Forward{}).
...
func (r *Repository) CreateForwardTx(userID int64, userName, name string, tunnelID int64, mode, remoteAddr, sniRules, strategy string, now int64, inx int, entryNodeIDs []int64, port int, inIp string, speedID interface{}) (int64, error) {
	if r == nil || r.db == nil {
		return 0, errors.New("repository not initialized")
	}
	var forwardID int64
...
func (r *Repository) CreateTunnelTx(tx *gorm.DB, name string, trafficRatio float64, typeVal int, flow int64, now int64, status int, inIP interface{}, inx int, protocol, ipPreference string) (int64, error) {
